...
public RGB open () {
	...
	if (GTK.GTK4) {
		...
		handle = GTK3.gtk_color_chooser_dialog_new(buffer, parent.topHandle());
	}
	...
	if (GTK.GTK4) {
		...
		GTK3.gtk_color_chooser_set_rgba (handle, rgba);
		if (rgbs != null) {
			int colorsPerRow = 9;
			...
			GTK3.gtk_color_chooser_add_palette(handle, GTK.GTK_ORIENTATION_HORIZONTAL, colorsPerRow,
					rgbs.length, gdkRGBAS);
			GTK3.gtk_color_chooser_set_rgba (handle, rgba);


			if (GTK3.gtk_color_chooser_get_use_alpha(handle)) {
				GTK3.gtk_color_chooser_set_use_alpha (handle, false);
			}
			OS.g_free (gdkRGBAS);
		}
	}
	...
	if (GTK.GTK4 ? GTK4.gtk_color_dialog_get_modal(handle) : GTK.gtk_window_get_modal(handle)) {
	...
	if (GTK.GTK4) {
		...
		long gdkRGBA = SyncDialogUtil.run(display, new AsyncReadyCallback() {
			...
			public void async(long callback) {
				GTK4.gtk_color_dialog_choose_rgba(handle, parent.handle, initialColor, 0, callback, 0);
			}
	...
	if (success) {
		...
		if (!GTK.GTK4) {
			rgba = new GdkRGBA();
			GTK3.gtk_color_chooser_get_rgba(handle, rgba);
		}
