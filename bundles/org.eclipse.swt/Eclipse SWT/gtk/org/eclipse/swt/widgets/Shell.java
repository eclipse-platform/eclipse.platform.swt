...
Point monitorOrigin () {
	...
	// A child Shell is positioned relative to its parent, and while still hidden its own
	// monitor is not even known, so the root toplevel is the anchor for the whole tree.
	if (parent != null) return parent.monitorOrigin ();
	return display.monitorOrigin (gtk_widget_get_window (topHandle ()));
}
