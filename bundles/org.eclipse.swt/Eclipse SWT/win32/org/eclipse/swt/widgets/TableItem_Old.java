 ...
 */
public class TableItem_Old extends Item {
	Table_Old parent;
	String[] strings;
	Image[] images;
	Font font;
	...
	static {
		DPIZoomChangeRegistry.registerHandler(TableItem_Old::handleDPIChange,
				TableItem_Old.class);
	}
 ...
 * @see Widget#getStyle
 */
public TableItem_Old(Table_Old parent, int style) {
 ...
 * @see Widget#getStyle
 */
public TableItem_Old(Table_Old parent, int style, int index) {
	this (parent, style, index, true);
}

TableItem_Old(Table_Old parent, int style, int index, boolean create) {
	super (parent, style);
	this.parent = parent;
	if (create) parent.createItem (this, index);
}

static Table_Old checkNull(Table_Old control) {
		if (control == null)
	...
	RECT getBounds(int row, int column, boolean getText, boolean getImage,
			boolean fullText, boolean fullImage, long hDC) {
		if (!getText && !getImage)
			return new RECT();
		...
		int bits = (int) OS.SendMessage(hwnd, OS.LVM_GETEXTENDEDLISTVIEWSTYLE,
				0, 0);
		if (column == 0 && (bits & OS.LVS_EX_FULLROWSELECT) == 0) {
			if (parent.explorerTheme) {
				...
				if (getText) {
					...
					if (hFont == -1 && hDC == 0) {
						TCHAR buffer = new TCHAR(parent.getCodePage(), text,
								true);
						width = (int) OS.SendMessage(hwnd,
								OS.LVM_GETSTRINGWIDTH, 0, buffer);
					} else {
						char[] buffer = text.toCharArray();
						long textDC = hDC != 0 ? hDC : OS.GetDC(hwnd),
								oldFont = -1;
						if (hDC == 0) {
							if (hFont == -1)
								hFont = OS.SendMessage(hwnd, OS.WM_GETFONT, 0,
										0);
						...
						int flags = OS.DT_NOPREFIX | OS.DT_SINGLELINE
								| OS.DT_CALCRECT;
						OS.DrawText(textDC, buffer, buffer.length, textRect,
								flags);
					...
					rect.right += width + Table_Old.INSET * 2;
				}
			} else {
				if (getText) {
					rect.left = OS.LVIR_SELECTBOUNDS;
					parent.ignoreCustomDraw = true;
					long code = OS.SendMessage(hwnd, OS.LVM_GETITEMRECT, row,
							rect);
					parent.ignoreCustomDraw = false;
					if (code == 0)
						return new RECT();
					if (!getImage) {
						...
						parent.ignoreCustomDraw = true;
						code = OS.SendMessage(hwnd, OS.LVM_GETITEMRECT, row,
								iconRect);
						parent.ignoreCustomDraw = false;
						if (code != 0)
			 ...
			 */
			boolean hasImage = (column == 0 && image != null)
					|| (images != null && images[column] != null);
			rect.top = column;
			if (fullText || fullImage || hDC == 0) {
				/*
				 * Bug in Windows. Despite the fact that the documentation
				 * states that LVIR_BOUNDS and LVIR_LABEL are identical when
				 * used with LVM_GETSUBITEMRECT, LVIR_BOUNDS can return a zero
				 * height. The fix is to use LVIR_LABEL.
				 */
				rect.left = getText ? OS.LVIR_LABEL : OS.LVIR_ICON;
				parent.ignoreCustomDraw = true;
				long code = OS.SendMessage(hwnd, OS.LVM_GETSUBITEMRECT, row,
						rect);
				parent.ignoreCustomDraw = false;
				if (code == 0)
					return new RECT();
				/*
				 * Feature in Windows. Calling LVM_GETSUBITEMRECT with
				 * LVIR_LABEL and zero for the column number gives the bounds of
				 * the first item without including the bounds of the icon. This
				 * is undocumented. When called with values greater than zero,
				 * the icon bounds are included and this behavior is documented.
				 * If the icon is needed in the bounds of the first item, the
				 * fix is to adjust the item bounds using the icon bounds.
				 */
				if (column == 0 && getText && getImage) {
					RECT iconRect = new RECT();
					iconRect.left = OS.LVIR_ICON;
					parent.ignoreCustomDraw = true;
					code = OS.SendMessage(hwnd, OS.LVM_GETSUBITEMRECT, row,
							iconRect);
					parent.ignoreCustomDraw = false;
					if (code != 0)
				...
				if (hasImage) {
					if (column != 0 && getText && !getImage) {
						...
						iconRect.top = column;
						iconRect.left = OS.LVIR_ICON;
						if (OS.SendMessage(hwnd, OS.LVM_GETSUBITEMRECT, row,
								iconRect) != 0) {
							rect.left = iconRect.right + Table_Old.INSET / 2;
						}
					}
				} else {
					if (getImage && !getText)
				...
				if (column == 0 && fullImage) {
					RECT headerRect = new RECT();
					long hwndHeader = OS.SendMessage(hwnd, OS.LVM_GETHEADER, 0,
							0);
					OS.SendMessage(hwndHeader, OS.HDM_GETITEMRECT, 0,
							headerRect);
					OS.MapWindowPoints(hwndHeader, hwnd, headerRect, 2);
					rect.left = headerRect.left;
				}
				...
				if (getText) {
					String string = column == 0
							? text
							: strings != null ? strings[column] : null;
					if (string != null) {
						RECT textRect = new RECT();
						char[] buffer = string.toCharArray();
						int flags = OS.DT_NOPREFIX | OS.DT_SINGLELINE
								| OS.DT_CALCRECT;
						OS.DrawText(hDC, buffer, buffer.length, textRect,
								flags);
		...
		/*
		 * Bug in Windows. In version 5.80 of COMCTL32.DLL, the top of the
		 * rectangle returned by LVM_GETSUBITEMRECT is off by the grid width
		 * when the grid is visible. The fix is to move the top of the rectangle
		 * up by the grid width.
		 */
		int gridWidth = parent.getLinesVisible() ? Table_Old.GRID_WIDTH : 0;
		rect.top -= gridWidth;
		if (column != 0)
			rect.left += gridWidth;
	...
	Rectangle getTextBoundsInPixels(int index) {
		...
		rect.left += 2;
		if (index != 0)
			rect.left += Table_Old.INSET;
		rect.left = Math.min(rect.left, rect.right);
		rect.right = rect.right - Table_Old.INSET;
	...
	public void setFont(Font font) {
		...
		Font newFont = (font == null
				? font
				: Font.win32_new(font, shell.nativeZoom));
		if (oldFont == newFont)
			return;
		this.font = newFont;
		 ...
		 * changed, causing Windows to flush cached bounds.
		 */
	...
	public void setFont(int index, Font font) {
		...
		if (oldFont == font)
			return;
		cellFont[index] = font == null
				? font
				: Font.win32_new(font, nativeZoom);
	 ...
	 * @exception IllegalArgumentException
	 *                <ul>
	 *                <li>ERROR_NULL_ARGUMENT - if the array of images is
	 *                null</li>
	...
	public void setImage(int index, Image image) {
		...
		if (index == 0)
			parent.setScrollWidth(this, false);
		boolean drawText = (image == null && oldImage != null)
				|| (image != null && oldImage == null);
	...
	private static void handleDPIChange(Widget widget, int newZoom,
			float scalingFactor) {
		if (!(widget instanceof TableItem_Old tableItem)) {
			return;
		}
		...
		if (cellFonts != null) {
			...
			for (int index = 0; index < cellFonts.length; index++) {
				...
				cellFonts[index] = cellFont == null
						? null
						: Font.win32_new(cellFont, shell.nativeZoom);
			}
		}
	}
}
