 ...
 */
public class TableColumn_Old extends Item {
	Table_Old parent;
	...
	static {
		DPIZoomChangeRegistry.registerHandler(TableColumn_Old::handleDPIChange,
				TableColumn_Old.class);
	}
	 ...
	 * @see Widget#getStyle
	 */
	public TableColumn_Old(Table_Old parent, int style) {
	 ...
	 * @see Widget#getStyle
	 */
	public TableColumn_Old(Table_Old parent, int style, int index) {
		super(parent, checkStyle(style));
		resizable = true;
		this.parent = parent;
	 ...
	 *                </ul>
	 *
	 * @see Table_Old#getColumnOrder()
	 * @see Table_Old#setColumnOrder(int[])
	 * @see TableColumn_Old#setMoveable(boolean)
	 * @see SWT#Move
	 *
	 * @since 3.1
	 */
	...
	private int calcAutoWidth(int index, boolean withHeader) {
		...
		// On the other hand, if control is invisible, 'WM_SETREDRAW' is not
		// needed.
		...
		try {
			...
			int oldWidth = (int) OS.SendMessage(hwnd, OS.LVM_GETCOLUMNWIDTH,
					index, 0);

			/*
			 * Feature in Windows. When LVSCW_AUTOSIZE_USEHEADER is used with
			 * LVM_SETCOLUMNWIDTH to resize the last column, the last column is
			 * expanded to fill the client area. The fix is to resize the table
			 * to be small, set the column width and then restore the table to
			 * its original size.
			 *
			 * Note: temporarily setting LVS_EX_COLUMNSNAPPOINTS may be a less
			 * intrusive workaround.
			 */
			RECT rect = null;
			...
			if (fixWidth) {
				...
				int flags = OS.SWP_NOACTIVATE | OS.SWP_NOMOVE | OS.SWP_NOREDRAW
						| OS.SWP_NOZORDER;
				OS.SetWindowPos(hwnd, 0, 0, 0, 0, rect.bottom - rect.top,
						flags);
			}

			int resizeType = withHeader
					? OS.LVSCW_AUTOSIZE_USEHEADER
					: OS.LVSCW_AUTOSIZE;
			OS.SendMessage(hwnd, OS.LVM_SETCOLUMNWIDTH, index, resizeType);

			if (fixWidth) {
				int flags = OS.SWP_NOACTIVATE | OS.SWP_NOMOVE | OS.SWP_NOZORDER;
				OS.SetWindowPos(hwnd, 0, 0, 0, rect.right - rect.left,
						rect.bottom - rect.top, flags);
			}

			int newWidth = (int) OS.SendMessage(hwnd, OS.LVM_GETCOLUMNWIDTH,
					index, 0);
	...
	public void pack() {
		...
		if (index == -1)
			return;
		long hwnd = parent.handle;
		int oldWidth = (int) OS.SendMessage(hwnd, OS.LVM_GETCOLUMNWIDTH, index,
				0);
		TCHAR buffer = new TCHAR(parent.getCodePage(), text, true);
		int headerWidth = (int) OS.SendMessage(hwnd, OS.LVM_GETSTRINGWIDTH, 0,
				buffer) + Table_Old.HEADER_MARGIN;
		if (OS.IsAppThemed())
			headerWidth += Table_Old.HEADER_EXTRA;
		boolean hasHeaderImage = false;
		if (image != null || parent.sortColumn == this) {
			hasHeaderImage = true;
			if (parent.sortColumn == this && parent.sortDirection != SWT.NONE) {
				headerWidth += Table_Old.SORT_WIDTH;
			} else if (image != null) {
				Rectangle bounds = image.getBoundsInPixels();
				headerWidth += bounds.width;
			}
			long hwndHeader = OS.SendMessage(hwnd, OS.LVM_GETHEADER, 0, 0);
			int margin = (int) OS.SendMessage(hwndHeader,
		...
		if (parent.hooks(SWT.MeasureItem)) {
			...
			long oldFont = 0,
					newFont = OS.SendMessage(hwnd, OS.WM_GETFONT, 0, 0);
			if (newFont != 0)
				oldFont = OS.SelectObject(hDC, newFont);
			int count = (int) OS.SendMessage(hwnd, OS.LVM_GETITEMCOUNT, 0, 0);
			for (int i = 0; i < count; i++) {
				TableItem_Old item = parent._getItem(i, false);
				if (item != null) {
					long hFont = item.fontHandle(index);
					if (hFont != -1)
						hFont = OS.SelectObject(hDC, hFont);
					Event event = parent.sendMeasureItemEvent(item, i, index,
							hDC);
					if (hFont != -1)
						hFont = OS.SelectObject(hDC, hFont);
					...
					columnWidth = Math.max(columnWidth,
							DPIUtil.scaleUp(bounds.x + bounds.width, getZoom())
									- headerRect.left);
				}
			}
			if (newFont != 0)
			...
			if (index == 0) {
				/*
				 * Bug in Windows. When LVM_SETCOLUMNWIDTH is used with
				 * LVSCW_AUTOSIZE where each item has I_IMAGECALLBACK but there
				 * are no images in the table, the size computed by
				 * LVM_SETCOLUMNWIDTH is too small for the first column, causing
				 * long items to be clipped with '...'. The fix is to increase
				 * the column width by a small amount.
				 */
				if (parent.imageList == null)
					columnWidth += 2;
				/*
				 * Bug in Windows. When LVM_SETCOLUMNWIDTH is used with
				 * LVSCW_AUTOSIZE for a table with a state image list, the
				 * column is width does not include space for the state icon.
				 * The fix is to increase the column width by the width of the
				 * image list.
				 */
				if ((parent.style & SWT.CHECK) != 0) {
					long hStateList = OS.SendMessage(hwnd, OS.LVM_GETIMAGELIST,
							OS.LVSIL_STATE, 0);
					if (hStateList != 0) {
		...
		if (headerWidth > columnWidth) {
			if (!hasHeaderImage) {
				// The code has been there for years and it's no longer clear
				// why
		...
		if (oldWidth != columnWidth) {
			...
			if (isDisposed())
				return;
			boolean moved = false;
			TableColumn_Old[] columns = parent.getColumns();
			for (int columnindex : parent.getColumnOrder()) {
				TableColumn_Old column = columns[columnindex];
	...
	public void setAlignment(int alignment) {
		...
		/*
		 * Bug in Windows. When LVM_SETCOLUMN is used to change the alignment of
		 * a column, the column is not redrawn to show the new alignment. The
		 * fix is to compute the visible rectangle for the column and redraw it.
		 */
		if (index != 0) {
			parent.forceResize();
	 ...
	 * @see Table_Old#setColumnOrder(int[])
	 * @see Table_Old#getColumnOrder()
	 * @see TableColumn_Old#getMoveable()
	 * @see SWT#Move
	 *
	 * @since 3.1
	 */
	...
	void setSortDirection(int direction) {
		...
		/*
		 * Bug in Windows. When LVM_SETSELECTEDCOLUMN is used to specify a
		 * selected column, Windows does not redraw either the new or the
		 * previous selected column. The fix is to force a redraw of both.
		 *
		 * Feature in Windows. When LVM_SETBKCOLOR is used with CLR_NONE and
		 * LVM_SETSELECTEDCOLUMN is used to select a column, Windows fills the
		 * column with the selection color, drawing on top of the background
		 * image and any other custom drawing. The fix is to avoid setting the
		 * selected column.
		 */
		parent.forceResize();
		RECT rect = new RECT();
		OS.GetClientRect(hwnd, rect);
		if ((int) OS.SendMessage(hwnd, OS.LVM_GETBKCOLOR, 0,
				0) != OS.CLR_NONE) {
			int oldColumn = (int) OS.SendMessage(hwnd, OS.LVM_GETSELECTEDCOLUMN,
					0, 0);
			int newColumn = direction == SWT.NONE ? -1 : index;
			OS.SendMessage(hwnd, OS.LVM_SETSELECTEDCOLUMN, newColumn, 0);
			RECT headerRect = new RECT();
			if (oldColumn != -1) {
				if (OS.SendMessage(hwndHeader, OS.HDM_GETITEMRECT, oldColumn,
						headerRect) != 0) {
					OS.MapWindowPoints(hwndHeader, hwnd, headerRect, 2);
					rect.left = headerRect.left;
					rect.right = headerRect.right;
		...
		RECT headerRect = new RECT();
		if (OS.SendMessage(hwndHeader, OS.HDM_GETITEMRECT, index,
				headerRect) != 0) {
			OS.MapWindowPoints(hwndHeader, hwnd, headerRect, 2);
			rect.left = headerRect.left;
			rect.right = headerRect.right;
	...
	public void setText(String string) {
		...
		/*
		 * Bug in Windows. For some reason, when the title of a column is
		 * changed after the column has been created, the alignment must also be
		 * reset or the text does not draw. The fix is to query and then set the
		 * alignment.
		 */
		long hwnd = parent.handle;
		LVCOLUMN lvColumn = new LVCOLUMN();
		...
		/*
		 * Bug in Windows. When a column header contains a mnemonic character,
		 * Windows does not measure the text properly. This causes '...' to
		 * always appear at the end of the text. The fix is to remove mnemonic
		 * characters.
		 */
	...
	private static void handleDPIChange(Widget widget, int newZoom,
			float scalingFactor) {
		if (!(widget instanceof TableColumn_Old tableColumn)) {
			return;
		}
