 ...
 */
public class Table_Old extends Composite {
	TableItem_Old[] items;
	int[] keys;
	TableColumn_Old[] columns;
	int columnCount, customCount, keyCount;
	ImageList imageList, headerImageList;
	TableItem_Old currentItem;
	TableColumn_Old sortColumn;
	RECT focusRect;
	...
	static {
		...
		DPIZoomChangeRegistry.registerHandler(Table_Old::handleDPIChange,
				Table_Old.class);
	}
	 ...
	 * @see Widget#getStyle
	 */
	public Table_Old(Composite parent, int style) {
	...
	boolean _checkGrow(int count) {
		...
		if (keys == null) {
			if (count == items.length) {
				 ...
				 * visible. When the table is painted, the items array is
				 * resized to be smaller to reduce memory usage.
				 */
				boolean small = getDrawing() && OS.IsWindowVisible(handle);
				int length = small
						? items.length + 4
						: Math.max(4, items.length * 3 / 2);
				TableItem_Old[] newItems = new TableItem_Old[length];
				System.arraycopy(items, 0, newItems, 0, items.length);
				items = newItems;
			}
		} else {
			// TODO - don't shrink when count is very small (ie. 2 or 4
			// elements)?
			// TODO - why? if setItemCount(1000000) is used after a shrink, then
			// we won't compress
			...
			if (!ignoreShrink && keyCount > count / 2) {
				...
				// TODO - experiment to determine an optimal growth rate for
				// keys
				if (keyCount == keys.length) {
					boolean small = getDrawing() && OS.IsWindowVisible(handle);
					int length = small
							? keys.length + 4
							: Math.max(4, keys.length * 3 / 2);
	...
	void _checkShrink() {
		...
		if (keys == null) {
			if (!ignoreShrink) {
				...
				int count = (int) OS.SendMessage(handle, OS.LVM_GETITEMCOUNT, 0,
						0);

				/*
				 * Bug in Windows. Call to OS.LVM_GETITEMCOUNT unexpectedly
				 * returns zero, leading to a possible
				 * "ArrayIndexOutOfBoundsException: 4" in SWT table. So, double
				 * check for any existing living items in the table and fixing
				 * the count value. Refer bug 292199.
				 */
				if (count == 0 && items.length > 4) {
					while (count < items.length && items[count] != null
							&& !items[count].isDisposed()) {
						count++;
					}
				}
	...
	TableItem_Old _getItem(int index) {
		return _getItem(index, true);
	}

	// TODO - check senders who have count (watch methods that change the count)
	TableItem_Old _getItem(int index, boolean create) {
		return _getItem(index, create, -1);
	}

	TableItem_Old _getItem(int index, boolean create, int count) {
		// TODO - code could be shared but it would mix keyed and non-keyed
		// logic
		if (keys == null) {
			...
			if (items[index] != null)
				return items[index];
			return items[index] = new TableItem_Old(this, SWT.NONE, -1, false);
		} else {
			if ((style & SWT.VIRTUAL) == 0 || !create) {
				if (keyCount == 0)
					return null;
			...
			if (keyIndex < 0) {
				if (count == -1) {
					count = (int) OS.SendMessage(handle, OS.LVM_GETITEMCOUNT, 0,
							0);
				}
				// TODO - _checkGrow() doesn't return a value, check keys ==
				// null instead
				if (_checkGrow(count)) {
					if (items[index] != null)
						return items[index];
					return items[index] = new TableItem_Old(this, SWT.NONE, -1,
							false);
				}
				keyIndex = -keyIndex - 1;
			...
			return items[keyIndex] = new TableItem_Old(this, SWT.NONE, -1,
					false);
		}
	}

	void _getItems(TableItem_Old[] result, int count) {
		if (keys == null) {
	...
	void _insertItem(int index, TableItem_Old item, int count) {
		if (keys == null) {
	...
	void _removeItems(int start, int index, int count) {
		if (keys == null) {
			...
			for (int i = keyCount - (right - left); i < keyCount; i++)
				keys[i] = 0;
			System.arraycopy(items, right, items, left, keyCount - right);
			for (int i = keyCount - (right - left); i < keyCount; i++)
				items[i] = null;
			keyCount -= (right - left);
			for (int i = left; i < keyCount; i++)
				keys[i] -= (right - left);
		}
	}
	...
	long callWindowProc(long hwnd, int msg, long wParam, long lParam,
		...
		switch (msg) {
			/* Keyboard messages */
			/*
			 * Feature in Windows. Windows sends LVN_ITEMACTIVATE from
			 * WM_KEYDOWN instead of WM_CHAR. This means that application code
			 * that expects to consume the key press and therefore avoid a
			 * SWT.DefaultSelection event will fail. The fix is to ignore
			 * LVN_ITEMACTIVATE when it is caused by WM_KEYDOWN and send
			 * SWT.DefaultSelection from WM_CHAR.
			 */
			case OS.WM_KEYDOWN :
				checkActivate = true;
				// FALL THROUGH
			...
			case OS.WM_WINDOWPOSCHANGED :
				...
				if (redraw) {
					 ...
					 * CLR_NONE to make the background of the table transparent,
					 * drawing becomes slow. The fix is to temporarily clear
					 * CLR_NONE when redraw is turned off.
					 */
			...
			case OS.WM_TIMER : {
				if (findImageControl() != null) {
					topIndex = (int) OS.SendMessage(handle, OS.LVM_GETTOPINDEX,
							0, 0);
				}
			}
		}
		...
		/*
		 * Bug in Windows. For some reason, when the WS_EX_COMPOSITED style is
		 * set in a parent of a table and the header is visible, Windows issues
		 * an endless stream of WM_PAINT messages. The fix is to call
		 * BeginPaint() and EndPaint() outside of WM_PAINT and pass the paint
		 * HDC in to the window proc.
		 */
			...
			switch (msg) {
				...
				case OS.WM_WINDOWPOSCHANGING : {
					...
					if ((style & SWT.H_SCROLL) == 0
							&& (bits & OS.WS_HSCROLL) != 0) {
						fixScroll = true;
						bits &= ~OS.WS_HSCROLL;
					}
					if ((style & SWT.V_SCROLL) == 0
							&& (bits & OS.WS_VSCROLL) != 0) {
						fixScroll = true;
						bits &= ~OS.WS_VSCROLL;
					}
					if (fixScroll)
		...
		switch (msg) {
			...
			case OS.WM_WINDOWPOSCHANGED :
				if (redraw) {
					...
					long hwndHeader = OS.SendMessage(handle, OS.LVM_GETHEADER,
							0, 0);
					if (hwndHeader != 0)
			...
			case OS.WM_TIMER : {
				if (findImageControl() != null) {
					if (topIndex != OS.SendMessage(handle, OS.LVM_GETTOPINDEX,
							0, 0)) {
						OS.InvalidateRect(handle, null, true);
					}
				}
	...
	LRESULT CDDS_ITEMPOSTPAINT(NMLVCUSTOMDRAW nmcd, long wParam, long lParam) {
		 ...
		 * LVS_EX_FULLROWSELECT and LVM_SETBKCOLOR is used with CLR_NONE to make
		 * the table transparent, Windows fills a black rectangle around any
		 * column that contains an image. The fix is clear LVS_EX_FULLROWSELECT
		 * during custom draw.
		 *
		 * NOTE: Since CDIS_FOCUS is cleared during custom draw, it is necessary
		 * to draw the focus rectangle after the item has been drawn.
		 */
		if (!ignoreCustomDraw && !ignoreDrawFocus && nmcd.left != nmcd.right) {
			if (OS.IsWindowVisible(handle) && OS.IsWindowEnabled(handle)) {
				if (!explorerTheme && (style & SWT.FULL_SELECTION) != 0) {
					if ((int) OS.SendMessage(handle, OS.LVM_GETBKCOLOR, 0,
							0) == OS.CLR_NONE) {
						...
						if ((dwExStyle & OS.LVS_EX_FULLROWSELECT) == 0) {
							...
							if (OS.SendMessage(handle, OS.LVM_GETNEXTITEM, -1,
									OS.LVNI_FOCUSED) == nmcd.dwItemSpec) {
								if (handle == OS.GetFocus()) {
									...
									if ((uiState & OS.UISF_HIDEFOCUS) == 0) {
										...
										long hwndHeader = OS.SendMessage(handle,
												OS.LVM_GETHEADER, 0, 0);
										int index = (int) OS.SendMessage(
												hwndHeader, OS.HDM_ORDERTOINDEX,
												0, 0);
										RECT itemRect = new RECT();
										if (index == 0) {
											itemRect.left = OS.LVIR_LABEL;
											OS.SendMessage(handle,
													OS.LVM_GETITEMRECT, index,
													itemRect);
										} else {
											itemRect.top = index;
											itemRect.left = OS.LVIR_ICON;
											OS.SendMessage(handle,
													OS.LVM_GETSUBITEMRECT,
													nmcd.dwItemSpec, itemRect);
										}
	...
	LRESULT CDDS_ITEMPREPAINT(NMLVCUSTOMDRAW nmcd, long wParam, long lParam) {
		 ...
		 * during custom draw.
		 *
		 * NOTE: It is also necessary to clear CDIS_FOCUS to stop the table from
		 * drawing the focus rectangle around the first item instead of the full
		 * row.
		 */
		if (!ignoreCustomDraw) {
			if (OS.IsWindowVisible(handle) && OS.IsWindowEnabled(handle)) {
					...
					if ((int) OS.SendMessage(handle, OS.LVM_GETBKCOLOR, 0,
						...
						if ((dwExStyle & OS.LVS_EX_FULLROWSELECT) == 0) {
							if ((nmcd.uItemState & OS.CDIS_FOCUS) != 0) {
								...
								OS.MoveMemory(lParam, nmcd,
										NMLVCUSTOMDRAW.sizeof);
							}
						}
					}
				}
		...
		if (explorerTheme && !ignoreCustomDraw) {
			hotIndex = (nmcd.uItemState & OS.CDIS_HOT) != 0
					? (int) nmcd.dwItemSpec
					: -1;
			if (hooks(SWT.EraseItem) && nmcd.left != nmcd.right) {
				OS.SaveDC(nmcd.hdc);
			...
			if (!explorerTheme && (style & SWT.FULL_SELECTION) != 0) {
				if ((int) OS.SendMessage(handle, OS.LVM_GETBKCOLOR, 0,
					...
					if ((dwExStyle & OS.LVS_EX_FULLROWSELECT) == 0) {
						...
						/*
						 * Feature in Windows. When LVM_SETEXTENDEDLISTVIEWSTYLE
						 * is used to set or clear the extended style bits and
						 * the table has a tooltip, the tooltip is hidden. The
						 * fix is to clear the tooltip before setting the bits
						 * and then reset it.
						 */
						long hwndToolTip = OS.SendMessage(handle,
								OS.LVM_SETTOOLTIPS, 0, 0);
						...
						OS.DeleteObject(rgn);
						/*
						 * Bug in Windows. Despite the documentation,
						 * LVM_SETTOOLTIPS uses WPARAM instead of LPARAM for the
						 * new tooltip The fix is to put the tooltip in both
						 * parameters.
						 */
						hwndToolTip = OS.SendMessage(handle, OS.LVM_SETTOOLTIPS,
	...
	LRESULT CDDS_PREPAINT(NMLVCUSTOMDRAW nmcd, long wParam, long lParam) {
		if (ignoreCustomDraw) {
			return new LRESULT(
					OS.CDRF_NOTIFYITEMDRAW | OS.CDRF_NOTIFYPOSTPAINT);
		}
		/*
		 * Bug in Windows. When the table has the extended style
		 * LVS_EX_FULLROWSELECT and LVM_SETBKCOLOR is used with CLR_NONE to make
		...
		if (OS.IsWindowVisible(handle)) {
			/*
			 * Feature in Windows. On Vista using the explorer theme, Windows
			 * draws a vertical line to separate columns. When there is only a
			 * single column, the line looks strange. The fix is to draw the
			 * background using custom draw.
			 */
			RECT rect = new RECT();
			...
			if (explorerTheme && columnCount == 0) {
				...
				if (control != null && control.backgroundImage != null) {
					...
					if (enabled
							&& (int) OS.SendMessage(handle, OS.LVM_GETBKCOLOR,
									0, 0) == OS.CLR_NONE
							|| !enabled && hasCustomBackground()) {
						if (control == null)
							control = this;
						fillBackground(nmcd.hdc, control.getBackgroundPixel(),
								rect);
						if (OS.IsAppThemed()) {
							if (sortColumn != null
								...
								if (index != -1) {
									...
									if (OS.SendMessage(hwndHeader,
										...
										if (OS.IntersectRect(columnRect,
												columnRect, rect)) {
											fillBackground(nmcd.hdc, clrSortBk,
													columnRect);
										}
									}
								}
	...
	LRESULT CDDS_SUBITEMPOSTPAINT(NMLVCUSTOMDRAW nmcd, long wParam,
			long lParam) {
		if (ignoreCustomDraw)
			return null;
		...
		if (OS.IsWindowVisible(handle)) {
			...
			if ((int) OS.SendMessage(handle, OS.LVM_GETBKCOLOR, 0,
				...
				if ((sortDirection & (SWT.UP | SWT.DOWN)) != 0) {
					if (sortColumn != null && !sortColumn.isDisposed()) {
						int oldColumn = (int) OS.SendMessage(handle,
								OS.LVM_GETSELECTEDCOLUMN, 0, 0);
						if (oldColumn == -1) {
							int newColumn = indexOf(sortColumn);
							long rgn = OS.CreateRectRgn(0, 0, 0, 0);
							int result = OS.GetUpdateRgn(handle, rgn, true);
							OS.SendMessage(handle, OS.LVM_SETSELECTEDCOLUMN,
									newColumn, 0);
							OS.ValidateRect(handle, null);
							if (result != OS.NULLREGION)
								OS.InvalidateRgn(handle, rgn, true);
			...
			if (hooks(SWT.PaintItem)) {
				TableItem_Old item = _getItem((int) nmcd.dwItemSpec);
	...
	LRESULT CDDS_SUBITEMPREPAINT(NMLVCUSTOMDRAW nmcd, long wParam,
			long lParam) {
		long hDC = nmcd.hdc;
		if (explorerTheme && !ignoreCustomDraw && hooks(SWT.EraseItem)
				&& (nmcd.left != nmcd.right)) {
			OS.RestoreDC(hDC, -1);
		}
		/*
		 * Feature in Windows. When a new table item is inserted using
		 * LVM_INSERTITEM in a table that is transparent (ie. LVM_SETBKCOLOR has
		 * been called with CLR_NONE), TVM_INSERTITEM calls NM_CUSTOMDRAW before
		 * the new item has been added to the array. The fix is to check for
		 * null.
		 *
		...
		if (ignoreCustomDraw || (nmcd.left == nmcd.right)) {
			return new LRESULT(
					hFont == -1 ? OS.CDRF_DODEFAULT : OS.CDRF_NEWFONT);
		}
		...
		if (OS.IsWindowVisible(handle)) {
			...
			if (hooks(SWT.MeasureItem)) {
				measureEvent = sendMeasureItemEvent(item, (int) nmcd.dwItemSpec,
						nmcd.iSubItem, nmcd.hdc);
		...
		int clrText = item.cellForeground != null
				? item.cellForeground[nmcd.iSubItem]
				: -1;
		if (clrText == -1)
			clrText = item.foreground;
		int clrTextBk = item.cellBackground != null
				? item.cellBackground[nmcd.iSubItem]
				: -1;
		if (clrTextBk == -1)
		 ...
		 * Bug in Windows. When the table has the extended style
		 * LVS_EX_FULLROWSELECT and LVM_SETBKCOLOR is used with CLR_NONE to make
		 * the table transparent, Windows draws a black rectangle around any
		 * column that contains an image. The fix is emulate
		 * LVS_EX_FULLROWSELECT by drawing the selection.
		 */
		final boolean enabled = OS.IsWindowEnabled(handle);
		if (OS.IsWindowVisible(handle) && enabled) {
			if (!explorerTheme && !ignoreDrawSelection
					&& (style & SWT.FULL_SELECTION) != 0) {
				int bits = (int) OS.SendMessage(handle,
						OS.LVM_GETEXTENDEDLISTVIEWSTYLE, 0, 0);
				if ((bits & OS.LVS_EX_FULLROWSELECT) == 0) {
					/*
					 * Bug in Windows. For some reason, CDIS_SELECTED always
					 * set, even for items that are not selected. The fix is to
					 * get the selection state from the item.
					 */
					LVITEM lvItem = new LVITEM();
					lvItem.mask = OS.LVIF_STATE;
					lvItem.stateMask = OS.LVIS_SELECTED;
					lvItem.iItem = (int) nmcd.dwItemSpec;
					long result = OS.SendMessage(handle, OS.LVM_GETITEM, 0,
							lvItem);
					if ((result != 0
							&& (lvItem.state & OS.LVIS_SELECTED) != 0)) {
						int clrSelection = -1;
						if (nmcd.iSubItem == 0) {
							if (OS.GetFocus() == handle
									|| display.getHighContrast()) {
								clrSelection = OS
										.GetSysColor(OS.COLOR_HIGHLIGHT);
							} else {
								if ((style & SWT.HIDE_SELECTION) == 0) {
									clrSelection = OS
											.GetSysColor(OS.COLOR_3DFACE);
								}
							}
						} else {
							if (OS.GetFocus() == handle
									|| display.getHighContrast()) {
								clrText = OS
										.GetSysColor(OS.COLOR_HIGHLIGHTTEXT);
								clrTextBk = clrSelection = OS
										.GetSysColor(OS.COLOR_HIGHLIGHT);
							} else {
								if ((style & SWT.HIDE_SELECTION) == 0) {
									clrTextBk = clrSelection = OS
						...
						if (clrSelection != -1) {
							RECT rect = item.getBounds((int) nmcd.dwItemSpec,
									nmcd.iSubItem, true, nmcd.iSubItem != 0,
									true, false, hDC);
		...
		if (!ignoreDrawForeground) {
			 ...
			 * subsequent cells are drawn using the previous font, foreground
			 * and background colors. The fix is to set the all attributes when
			 * any attribute could have changed.
			 */
			boolean hasAttributes = true;
			if (hFont == -1 && clrText == -1 && clrTextBk == -1) {
				if (item.cellForeground == null && item.cellBackground == null
						&& item.cellFont == null) {
					int count = (int) OS.SendMessage(hwndHeader,
					...
					if (clrTextBk == -1) {
						...
						if (selectionForeground == -1) {
							...
							if (control.backgroundImage == null) {
								if ((int) OS.SendMessage(handle,
										OS.LVM_GETBKCOLOR, 0,
										0) != OS.CLR_NONE) {
									nmcd.clrTextBk = control
											.getBackgroundPixel();
								}
							}
						}
					} else {
						nmcd.clrTextBk = selectionForeground != -1
								? OS.CLR_NONE
								: clrTextBk;
					}
		...
		/*
		 * Feature in Windows. When there is a sort column, the sort column
		 * color draws on top of the background color for an item. The fix is to
		 * clear the sort column in CDDS_SUBITEMPREPAINT, and reset it in
		 * CDDS_SUBITEMPOSTPAINT.
		 *
		 * Update region is saved and restored around LVM_SETSELECTEDCOLUMN to
		 * prevent infinite WM_PAINT on Vista.
		 */
		if ((enabled && clrTextBk != -1)
				|| (!enabled && hasCustomBackground())) {
			int oldColumn = (int) OS.SendMessage(handle,
					OS.LVM_GETSELECTEDCOLUMN, 0, 0);
		...
		if (!enabled) {
			/*
			 * Feature in Windows. When the table is disabled, it draws with a
			 * gray background but does not gray the text. The fix is to
			 * explicitly gray the text, but only, when it wasn't customized.
			 */
	...
	boolean checkData(TableItem_Old item, boolean redraw) {
		if ((style & SWT.VIRTUAL) == 0)
			return true;
		return checkData(item, indexOf(item), redraw);
	}

	boolean checkData(TableItem_Old item, int index, boolean redraw) {
		if ((style & SWT.VIRTUAL) == 0)
			return true;
		if (!item.cached) {
			item.cached = true;
	...
	public void clear(int index) {
		...
		TableItem_Old item = _getItem(index, false);
		if (item != null) {
			if (item != currentItem)
				item.clear();
			/*
			 * Bug in Windows. Despite the fact that every item in the table
			 * always has LPSTR_TEXTCALLBACK, Windows caches the bounds for the
			 * selected items. This means that when you change the string to be
			 * something else, Windows correctly asks you for the new string but
			 * when the item is selected, the selection draws using the bounds
			 * of the previous item. The fix is to reset LPSTR_TEXTCALLBACK even
			 * though it has not changed, causing Windows to flush cached
			 * bounds.
			 */
	...
	public void clear(int start, int end) {
		...
		if (start == 0 && end == count - 1) {
			...
			for (int i = start; i <= end; i++) {
				TableItem_Old item = _getItem(i, false);
				if (item != null) {
					if (item != currentItem) {
						cleared = true;
						item.clear();
					 ...
					 * bounds for the selected items. This means that when you
					 * change the string to be something else, Windows correctly
					 * asks you for the new string but when the item is
					 * selected, the selection draws using the bounds of the
			...
			if (cleared) {
				...
				TableItem_Old item = start == end
						? _getItem(start, false)
						: null;
				setScrollWidth(item, false);
			}
		}
	...
	public void clear(int[] indices) {
		...
		for (int i = 0; i < indices.length; i++) {
			...
			if (item != null) {
				...
				/*
				 * Bug in Windows. Despite the fact that every item in the table
				 * always has LPSTR_TEXTCALLBACK, Windows caches the bounds for
				 * the selected items. This means that when you change the
				 * string to be something else, Windows correctly asks you for
				 * the new string but when the item is selected, the selection
				 * draws using the bounds of the previous item. The fix is to
				 * reset LPSTR_TEXTCALLBACK even though it has not changed,
				 * causing Windows to flush cached bounds.
				 */
				if ((style & SWT.VIRTUAL) == 0 && item.cached) {
					if (lvItem == null) {
						lvItem = new LVITEM();
	...
	Point computeSizeInPixels(int wHint, int hHint, boolean changed) {
		...
		// int oldFont = 0, newFont = OS.SendMessage (handle, OS.WM_GETFONT, 0,
		// 0);
		...
		if (wHint != SWT.DEFAULT) {
			...
			int width = 0;
			int count = (int) OS.SendMessage(hwndHeader, OS.HDM_GETITEMCOUNT, 0,
					0);
			for (int i = 0; i < count; i++) {
		...
		long result = OS.SendMessage(handle, OS.LVM_APPROXIMATEVIEWRECT, -1,
		...
		height += (int) OS.SendMessage(handle, OS.LVM_GETITEMCOUNT, 0, 0)
				* itemHeight;
		if (width == 0)
			width = DEFAULT_WIDTH;
		if (height == 0)
	...
	void createHandle() {
		...
		if ((style & SWT.CHECK) != 0) {
			long empty = OS.SendMessage(handle, OS.LVM_APPROXIMATEVIEWRECT, 0,
					0);
			long oneItem = OS.SendMessage(handle, OS.LVM_APPROXIMATEVIEWRECT, 1,
					0);
			int width = OS.HIWORD(oneItem) - OS.HIWORD(empty), height = width;
			setCheckboxImageList(width, height, false);
			OS.SendMessage(handle, OS.LVM_SETCALLBACKMASK,
					OS.LVIS_STATEIMAGEMASK, 0);
		}
		...
		if ((style & SWT.RIGHT_TO_LEFT) != 0) {
			...
			OS.SetWindowLong(hwndTooltip, OS.GWL_EXSTYLE,
					bits3 | OS.WS_EX_LAYOUTRTL);
		}
	}
	...
	void createHeaderToolTips() {
		...
		headerToolTipHandle = OS.CreateWindowEx(bits,
				new TCHAR(0, OS.TOOLTIPS_CLASS, true), null, OS.TTS_NOPREFIX,
				OS.CW_USEDEFAULT, 0, OS.CW_USEDEFAULT, 0, handle, 0,
				OS.GetModuleHandle(null), null);
		if (headerToolTipHandle == 0)
			error(SWT.ERROR_NO_HANDLES);
		maybeEnableDarkSystemTheme(headerToolTipHandle);
		/*
		 * Feature in Windows. Despite the fact that the tool tip text contains
		 * \r\n, the tooltip will not honour the new line unless
		 * TTM_SETMAXTIPWIDTH is set. The fix is to set TTM_SETMAXTIPWIDTH to a
		 * large value.
		 */
		OS.SendMessage(headerToolTipHandle, OS.TTM_SETMAXTIPWIDTH, 0, 0x7FFF);
	}

	void createItem(TableColumn_Old column, int index) {
		if (!(0 <= index && index <= columnCount))
			error(SWT.ERROR_INVALID_RANGE);
		int oldColumn = (int) OS.SendMessage(handle, OS.LVM_GETSELECTEDCOLUMN,
				0, 0);
		if (oldColumn >= index) {
			OS.SendMessage(handle, OS.LVM_SETSELECTEDCOLUMN, oldColumn + 1, 0);
		}
		if (columnCount == columns.length) {
			TableColumn_Old[] newColumns = new TableColumn_Old[columns.length
					+ 4];
		...
		for (int i = 0; i < itemCount; i++) {
			...
			if (item != null) {
				...
				if (strings != null) {
					...
					System.arraycopy(strings, index, temp, index + 1,
							columnCount - index);
					item.strings = temp;
				}
				...
				if (images != null) {
					...
					System.arraycopy(images, index, temp, index + 1,
							columnCount - index);
					item.images = temp;
				}
				if (index == 0) {
				...
				if (item.cellFont != null) {
					...
					System.arraycopy(cellFont, index, temp, index + 1,
							columnCount - index);
					item.cellFont = temp;
				}
			}
		 ...
		 */
		System.arraycopy(columns, index, columns, index + 1,
				columnCount++ - index);
		columns[index] = column;

		/*
		 * Ensure that resize listeners for the table and for columns within the
		 * table are not called. This can happen when the first column is
	...
	void createItem(TableItem_Old item, int index) {
		int count = (int) OS.SendMessage(handle, OS.LVM_GETITEMCOUNT, 0, 0);
		if (!(0 <= index && index <= count))
			error(SWT.ERROR_INVALID_RANGE);
		_checkGrow(count);
		...
		lvItem.pszText = OS.LPSTR_TEXTCALLBACK;
		/*
		 * Bug in Windows. Despite the fact that the image list index has never
		 * been set for the item, Windows always assumes that the image index
		 * for the item is valid. When an item is inserted, the image index is
		 * zero. Therefore, when the first image is inserted and is assigned
		 * image index zero, every item draws with this image. The fix is to set
		 * the image index when the the item is created.
		 */
		lvItem.iImage = OS.I_IMAGECALLBACK;
	...
	void destroyItem(TableColumn_Old column) {
		int index = 0;
		...
		if (oldColumn == index) {
			...
			if (oldColumn > index) {
				OS.SendMessage(handle, OS.LVM_SETSELECTEDCOLUMN, oldColumn - 1,
						0);
			}
		}
		int orderIndex = 0;
		...
		if (index == 0) {
			first = true;
			/*
			 * Changing the content of a column using LVM_SETCOLUMN causes the
			 * table control to send paint events. At this point the partially
			 * disposed column is still part of the table and paint handler can
			 * try to access it. This can cause exceptions. The fix is to turn
			 * redraw off.
			 */
			setRedraw(false);
			if (columnCount > 1) {
				...
				long hwndHeader = OS.SendMessage(handle, OS.LVM_GETHEADER, 0,
						0);
			...
			if ((style & SWT.VIRTUAL) == 0) {
				...
				lvItem.pszText = OS.LPSTR_TEXTCALLBACK;
				lvItem.iImage = OS.I_IMAGECALLBACK;
				int itemCount = (int) OS.SendMessage(handle,
		...
		if (first)
			index = 0;
		System.arraycopy(columns, index + 1, columns, index,
				--columnCount - index);
			...
			if (item != null) {
				if (columnCount == 0) {
					...
					if (item.strings != null) {
						...
						System.arraycopy(strings, 0, temp, 0, index);
						System.arraycopy(strings, index + 1, temp, index,
								columnCount - index);
						item.strings = temp;
					} else {
						if (index == 0)
					...
					if (item.images != null) {
						...
						System.arraycopy(images, 0, temp, 0, index);
						System.arraycopy(images, index + 1, temp, index,
								columnCount - index);
						item.images = temp;
					} else {
						if (index == 0)
					...
					if (item.cellBackground != null) {
						...
						System.arraycopy(cellBackground, 0, temp, 0, index);
						System.arraycopy(cellBackground, index + 1, temp, index,
								columnCount - index);
						item.cellBackground = temp;
					}
					if (item.cellForeground != null) {
						...
						int[] temp = new int[columnCount];
						System.arraycopy(cellForeground, 0, temp, 0, index);
						System.arraycopy(cellForeground, index + 1, temp, index,
								columnCount - index);
						item.cellForeground = temp;
					}
					if (item.cellFont != null) {
						...
						System.arraycopy(cellFont, 0, temp, 0, index);
						System.arraycopy(cellFont, index + 1, temp, index,
								columnCount - index);
						item.cellFont = temp;
					}
				}
		...
		if (columnCount != 0) {
			/*
			 * Bug in Windows. When LVM_DELETECOLUMN is used to delete a column
			 * zero when that column is both the first column in the table and
			 * the first column in the column order array, Windows incorrectly
			 * computes the new column order. For example, both the orders {0,
			 * 3, 1, 2} and {0, 3, 2, 1} give a new column order of {0, 2, 1},
			 * while {0, 2, 1, 3} gives {0, 1, 2, 3}. The fix is to compute the
			 * new order and compare it with the order that Windows is using. If
			 * the two differ, the new order is used.
			 */
			int count = 0;
			...
			if (j != newOrder.length) {
				OS.SendMessage(handle, OS.LVM_SETCOLUMNORDERARRAY,
						newOrder.length, newOrder);
				/*
				 * Bug in Windows. When LVM_SETCOLUMNORDERARRAY is used to
				 * change the column order, the header redraws correctly but the
				 * table does not. The fix is to force a redraw.
				 */
				OS.InvalidateRect(handle, null, true);
			}
			TableColumn_Old[] newColumns = new TableColumn_Old[columnCount
					- orderIndex];
			...
			for (TableColumn_Old newColumn : newColumns) {
	...
	void destroyItem(TableItem_Old item) {
		int count = (int) OS.SendMessage(handle, OS.LVM_GETITEMCOUNT, 0, 0);
		int index = 0;
		while (index < count) {
	 ...
	 * the order that they were created. If no <code>TableColumn_Old</code>s
	 * were created by the programmer, this method will throw
	 ...
	 * @see SWT#Move
	 */
	public TableColumn_Old getColumn(int index) {
		checkWidget();
		if (!(0 <= index && index < columnCount))
			error(SWT.ERROR_INVALID_RANGE);
	 ...
	 * <code>TableColumn_Old</code>s were created by the programmer, this value
	...
	/**
	 * Returns an array of <code>TableColumn_Old</code>s which are the columns
	 * in the receiver. Columns are returned in the order that they were
	 * created. If no <code>TableColumn_Old</code>s were created by the
	 ...
	 * @see SWT#Move
	 */
	public TableColumn_Old[] getColumns() {
		checkWidget();
		TableColumn_Old[] result = new TableColumn_Old[columnCount];
	...
	int getFocusIndex() {
		// checkWidget ();
		return (int) OS.SendMessage(handle, OS.LVM_GETNEXTITEM, -1,
				OS.LVNI_FOCUSED);
	}
	 ...
	 *                </ul>
	 */
	public TableItem_Old getItem(int index) {
		checkWidget();
		int count = (int) OS.SendMessage(handle, OS.LVM_GETITEMCOUNT, 0, 0);
		if (!(0 <= index && index < count))
			error(SWT.ERROR_INVALID_RANGE);
	 ...
	 *                </ul>
	 */
	public TableItem_Old getItem(Point point) {
		checkWidget();
		if (point == null)
			error(SWT.ERROR_NULL_ARGUMENT);
	...
	TableItem_Old getItemInPixels(Point point) {
		int count = (int) OS.SendMessage(handle, OS.LVM_GETITEMCOUNT, 0, 0);
		if (count == 0)
			return null;
		...
		if ((style & SWT.FULL_SELECTION) == 0) {
			if (hooks(SWT.MeasureItem)) {
				/*
				 * Bug in Windows. When LVM_SUBITEMHITTEST is used to hittest a
				 * point that is above the table, instead of returning -1 to
				 * indicate that the hittest failed, a negative index is
				 * returned. The fix is to consider any value that is negative a
				 * failure.
				 */
				if (OS.SendMessage(handle, OS.LVM_SUBITEMHITTEST, 0,
						pinfo) < 0) {
					RECT rect = new RECT();
					rect.left = OS.LVIR_ICON;
					ignoreCustomDraw = true;
					long code = OS.SendMessage(handle, OS.LVM_GETITEMRECT, 0,
							rect);
					ignoreCustomDraw = false;
					if (code != 0) {
						pinfo.x = rect.left;
						/*
						 * Bug in Windows. When LVM_SUBITEMHITTEST is used to
						 * hittest a point that is above the table, instead of
						 * returning -1 to indicate that the hittest failed, a
						 * negative index is returned. The fix is to consider
						 * any value that is negative a failure.
						 */
		...
		if (pinfo.iItem != -1) {
			/*
			 * Bug in Windows. When the point that is used by LVM_HITTEST is
			 * inside the header, Windows returns the first item in the table.
			 * The fix is to check when LVM_HITTEST returns the first item and
			 * make sure that when the point is within the header, the first
	 ...
	 *                </ul>
	 */
	public TableItem_Old[] getItems() {
		checkWidget();
		int count = (int) OS.SendMessage(handle, OS.LVM_GETITEMCOUNT, 0, 0);
		TableItem_Old[] result = new TableItem_Old[count];
		if ((style & SWT.VIRTUAL) != 0) {
			for (int i = 0; i < count; i++) {
				result[i] = _getItem(i);
			}
	...
	private boolean _getLinesVisible() {
		int bits = (int) OS.SendMessage(handle, OS.LVM_GETEXTENDEDLISTVIEWSTYLE,
				0, 0);
	 ...
	 *                </ul>
	 */
	public TableItem_Old[] getSelection() {
		checkWidget();
		int i = -1, j = 0, count = (int) OS.SendMessage(handle,
				OS.LVM_GETSELECTEDCOUNT, 0, 0);
		TableItem_Old[] result = new TableItem_Old[count];
		while ((i = (int) OS.SendMessage(handle, OS.LVM_GETNEXTITEM, i,
				OS.LVNI_SELECTED)) != -1) {
			result[j++] = _getItem(i);
		}
		return result;
	...
	public int getSelectionIndex() {
		checkWidget();
		int focusIndex = (int) OS.SendMessage(handle, OS.LVM_GETNEXTITEM, -1,
				OS.LVNI_FOCUSED);
		int selectedIndex = (int) OS.SendMessage(handle, OS.LVM_GETNEXTITEM, -1,
				OS.LVNI_SELECTED);
	...
	public int[] getSelectionIndices() {
		...
		int lastIndex = -1;
		for (int i = 0; i < count; i++) {
			lastIndex = (int) OS.SendMessage(handle, OS.LVM_GETNEXTITEM,
					lastIndex, OS.LVNI_SELECTED);
			if (lastIndex == -1) {
				break;
			}
	 ...
	 *                </ul>
	 *
	 * @see #setSortColumn(TableColumn_Old)
	 *
	 * @since 3.2
	 */
	public TableColumn_Old getSortColumn() {
		checkWidget();
		return sortColumn;
	}

	int getSortColumnPixel() {
		int pixel = OS.IsWindowEnabled(handle) || hasCustomBackground()
				? getBackgroundPixel()
	...
	public int getTopIndex() {
		 ...
		 */
		return Math.max(0,
				(int) OS.SendMessage(handle, OS.LVM_GETTOPINDEX, 0, 0));
	}
	...
	boolean hitTestSelection(int index, int x, int y) {
		...
		if (0 <= index && index < count) {
			TableItem_Old item = _getItem(index);
	...
	int imageIndex(Image image, int column) {
		...
		if (imageList == null) {
			Rectangle bounds = DPIUtil.scaleBounds(image.getBounds(),
					this.getZoom(), 100);
			imageList = display.getImageList(style & SWT.RIGHT_TO_LEFT,
					bounds.width, bounds.height, getZoom());
			int index = imageList.indexOf(image);
			if (index == -1)
				index = imageList.add(image);
			long hImageList = imageList.getHandle(getZoom());
			/*
			 * Bug in Windows. Making any change to an item that changes the
			 * item height of a table while the table is scrolled can cause the
			 * lines to draw incorrectly. This happens even when the lines are
			 * not currently visible and are shown afterwards. The fix is to
			 * save the top index, scroll to the top of the table and then
			 * restore the original top index.
			 */
			...
			if (headerImageList != null) {
				...
				OS.SendMessage(hwndHeader, OS.HDM_SETIMAGELIST, 0,
						hHeaderImageList);
			}
	 ...
	 *                </ul>
	 */
	public int indexOf(TableColumn_Old column) {
		checkWidget();
		if (column == null)
			error(SWT.ERROR_NULL_ARGUMENT);
	 ...
	 *                </ul>
	 */
	public int indexOf(TableItem_Old item) {
		checkWidget();
		if (item == null)
			error(SWT.ERROR_NULL_ARGUMENT);
	...
	void releaseChildren(boolean destroy) {
		...
		if (columns != null) {
			for (int i = 0; i < columnCount; i++) {
				TableColumn_Old column = columns[i];
	...
	public void remove(int start, int end) {
		...
		if (start == 0 && end == count - 1) {
			...
			if (index <= end)
				error(SWT.ERROR_ITEM_NOT_REMOVED);
			/*
			 * This code is intentionally commented. It is not necessary to
			 * check for an empty table because removeAll() was called when the
			 * start == 0 and end == count - 1.
			 */
	 ...
	 *                </ul>
	 *
	 * @see Table_Old#setSelection(int[])
	 */
	 ...
	 *                </ul>
	 *
	 * @see Table_Old#setSelection(int,int)
	 */
	...
	void sendEraseItemEvent(TableItem_Old item, NMLVCUSTOMDRAW nmcd,
			long lParam,
			Event measureEvent) {
		long hDC = nmcd.hdc;
		int clrText = item.cellForeground != null
		...
		if (clrTextBk == -1)
			clrTextBk = item.background;
		/*
		 * Bug in Windows. For some reason, CDIS_SELECTED always set, even for
		 * items that are not selected. The fix is to get the selection state
		 * from the item.
		 */
		LVITEM lvItem = new LVITEM();
		lvItem.mask = OS.LVIF_STATE;
		...
		if (OS.IsWindowEnabled(handle)) {
			if (selected && (nmcd.iSubItem == 0
				...
				/*
				 * Bug in Windows. When LVM_SETTEXTBKCOLOR, LVM_SETBKCOLOR or
				 * LVM_SETTEXTCOLOR is used to set the background color of the
				 * the text or the control, the color is not set in the HDC that
				 * is provided in Custom Draw. The fix is to explicitly set the
				 * color.
				 */
		...
		RECT cellRect = item.getBounds((int) nmcd.dwItemSpec, nmcd.iSubItem,
				true, true, true, true, hDC);
		Event event = new Event();
		event.item = item;
		event.gc = gc;
		...
		Rectangle bounds = DPIUtil.scaleDown(new Rectangle(cellRect.left,
				cellRect.top, cellRect.right - cellRect.left,
				cellRect.bottom - cellRect.top), getZoom());
		...
		if (!ignoreDrawHot || !ignoreDrawSelection || !ignoreDrawFocus
			...
			boolean fullText = (style & SWT.FULL_SELECTION) != 0
					|| !firstColumn;
			RECT textRect = item.getBounds((int) nmcd.dwItemSpec, nmcd.iSubItem,
					true, false, fullText, false, hDC);
			if ((style & SWT.FULL_SELECTION) == 0) {
				if (measureEvent != null) {
					Rectangle boundsInPixels = DPIUtil
							.scaleUp(measureEvent.getBounds(), getZoom());
					textRect.right = Math.min(cellRect.right,
							boundsInPixels.x + boundsInPixels.width);
				}
				if (!ignoreDrawFocus) {
					nmcd.uItemState &= ~OS.CDIS_FOCUS;
					OS.MoveMemory(lParam, nmcd, NMLVCUSTOMDRAW.sizeof);
			...
			if (explorerTheme) {
				boolean backgroundWanted = !ignoreDrawHot || drawDrophilited
						|| (!ignoreDrawSelection && clrSelectionBk != -1);

				if (backgroundWanted) {
					RECT pClipRect = new RECT();
					OS.SetRect(pClipRect, nmcd.left, nmcd.top, nmcd.right,
							nmcd.bottom);
					RECT rect = new RECT();
					OS.SetRect(rect, nmcd.left, nmcd.top, nmcd.right,
							nmcd.bottom);
					if ((style & SWT.FULL_SELECTION) != 0) {
						int count = (int) OS.SendMessage(hwndHeader,
								OS.HDM_GETITEMCOUNT, 0, 0);
						int index = (int) OS.SendMessage(hwndHeader,
								OS.HDM_ORDERTOINDEX, count - 1, 0);
						RECT headerRect = new RECT();
						OS.SendMessage(hwndHeader, OS.HDM_GETITEMRECT, index,
								headerRect);
						OS.MapWindowPoints(hwndHeader, handle, headerRect, 2);
						rect.right = headerRect.right;
						index = (int) OS.SendMessage(hwndHeader,
								OS.HDM_ORDERTOINDEX, 0, 0);
						OS.SendMessage(hwndHeader, OS.HDM_GETITEMRECT, index,
								headerRect);
						OS.MapWindowPoints(hwndHeader, handle, headerRect, 2);
		...
		if (ignoreDrawForeground) {
			RECT clipRect = item.getBounds((int) nmcd.dwItemSpec, nmcd.iSubItem,
					true, true, true, false, hDC);
			OS.SaveDC(hDC);
			OS.SelectClipRgn(hDC, 0);
			OS.ExcludeClipRect(hDC, clipRect.left, clipRect.top, clipRect.right,
					clipRect.bottom);
		}
	}

	Event sendEraseItemEvent(TableItem_Old item, NMTTCUSTOMDRAW nmcd,
			int column,
			RECT cellRect) {
		...
		event.gc = gc;
		event.detail |= SWT.FOREGROUND;
		event.setBounds(DPIUtil.scaleDown(new Rectangle(cellRect.left,
				cellRect.top, cellRect.right - cellRect.left,
				cellRect.bottom - cellRect.top), getZoom()));
	...
	Event sendMeasureItemEvent(TableItem_Old item, int row, int column,
			long hDC) {
		GCData data = new GCData();
		data.device = display;
		data.font = item.getFont(column);
		int nSavedDC = OS.SaveDC(hDC);
		GC gc = createNewGC(hDC, data);
		RECT itemRect = item.getBounds(row, column, true, true, false, false,
				hDC);
		Event event = new Event();
		event.item = item;
		event.gc = gc;
		event.index = column;
		event.setBounds(DPIUtil.scaleDown(new Rectangle(itemRect.left,
				itemRect.top, itemRect.right - itemRect.left,
				itemRect.bottom - itemRect.top), getZoom()));
		...
		if (OS.IsWindowEnabled(handle)) {
			...
			boolean selected = (result != 0
					&& (lvItem.state & OS.LVIS_SELECTED) != 0);
			if (selected
					&& (column == 0 || (style & SWT.FULL_SELECTION) != 0)) {
				if (OS.GetFocus() == handle || display.getHighContrast()) {
					drawSelected = true;
				} else {
					drawSelected = (style & SWT.HIDE_SELECTION) == 0;
		...
		OS.RestoreDC(hDC, nSavedDC);
		if (!isDisposed() && !item.isDisposed()) {
			Rectangle boundsInPixels = DPIUtil.scaleUp(event.getBounds(),
					getZoom());
			if (columnCount == 0) {
				int width = (int) OS.SendMessage(handle, OS.LVM_GETCOLUMNWIDTH,
						0, 0);
	...
	LRESULT sendMouseDownEvent(int type, int button, int msg, long wParam,
			long lParam) {
		...
		/*
		 * Feature in Windows. Inside WM_LBUTTONDOWN and WM_RBUTTONDOWN, the
		 * widget starts a modal loop to determine if the user wants to begin a
		 * drag/drop operation or marque select. Unfortunately, this modal loop
		 * eats the corresponding mouse up. The fix is to detect the cases when
		 * the modal loop has eaten the mouse up and issue a fake mouse up.
		 *
		 * By observation, when the mouse is clicked anywhere but the check box,
		 * the widget eats the mouse up. When the mouse is dragged, the widget
		 * does not eat the mouse up.
		 */
		LVHITTESTINFO pinfo = new LVHITTESTINFO();
			...
			if (hooks(SWT.MeasureItem)) {
				...
				if (OS.SendMessage(handle, OS.LVM_SUBITEMHITTEST, 0,
					...
					if (count != 0) {
						...
						rect.left = OS.LVIR_ICON;
						ignoreCustomDraw = true;
						long code = OS.SendMessage(handle, OS.LVM_GETITEMRECT,
								0, rect);
						ignoreCustomDraw = false;
						if (code != 0) {
							pinfo.x = rect.left;
							/*
							 * Bug in Windows. When LVM_SUBITEMHITTEST is used
							 * to hittest a point that is above the table,
							 * instead of returning -1 to indicate that the
							 * hittest failed, a negative index is returned. The
							 * fix is to consider any value that is negative a
							 * failure.
							 */
							OS.SendMessage(handle, OS.LVM_SUBITEMHITTEST, 0,
									pinfo);
							if (pinfo.iItem < 0)
								pinfo.iItem = -1;
							pinfo.flags &= ~(OS.LVHT_ONITEMICON
									| OS.LVHT_ONITEMLABEL);
						}
					}
				} else {
		 ...
		 * While not strictly wrong, this is unexpected. The fix is to detect
		 * the case and avoid calling the window proc.
		 */
		if ((style & SWT.SINGLE) != 0 || hooks(SWT.MouseDown)
				|| hooks(SWT.MouseUp)) {
			if (pinfo.iItem == -1) {
				if (!display.captureChanged && !isDisposed()) {
					if (OS.GetCapture() != handle)
				...
				/*
				 * We're skipping default processing, but at least set focus to
				 * control
				 */
				OS.SetFocus(handle);
				return LRESULT.ZERO;
		 ...
		 * single-select table, Windows does not issue a WM_NOTIFY because the
		 * item state has not changed. This is strictly correct but is
		 * inconsistent with the list widget and other widgets in Windows. The
		 * fix is to detect the case when an item is reselected and mark it as
		 * selected.
		 */
		...
		/*
		 * Determine whether the user has selected an item based on
		 * SWT.MeasureItem
		 */
		fullRowSelect = false;
		if (pinfo.iItem != -1) {
			if ((style & SWT.FULL_SELECTION) == 0) {
				if (hooks(SWT.MeasureItem)) {
					fullRowSelect = hitTestSelection(pinfo.iItem, pinfo.x,
							pinfo.y);
					if (fullRowSelect) {
		 ...
		 * Feature in Windows. Inside WM_LBUTTONDOWN and WM_RBUTTONDOWN, the
		 * widget starts a modal loop to determine if the user wants to begin a
		 * drag/drop operation or marque select. This modal loop eats mouse
		 * events until a drag is detected. The fix is to avoid this behavior by
		 * only running the drag and drop when the event is hooked and the mouse
		 * is over an item.
		 */
		...
		/*
		 * Temporarily set LVS_EX_FULLROWSELECT to allow drag and drop and the
		 * mouse to manipulate items based on the results of the SWT.MeasureItem
		 * event.
		 */
		if (fullRowSelect) {
	...
	void sendPaintItemEvent(TableItem_Old item, NMLVCUSTOMDRAW nmcd) {
		long hDC = nmcd.hdc;
		GCData data = new GCData();
		data.device = display;
		data.font = item.getFont(nmcd.iSubItem);
		...
		int nSavedDC = OS.SaveDC(hDC);
		GC gc = createNewGC(hDC, data);
		RECT itemRect = item.getBounds((int) nmcd.dwItemSpec, nmcd.iSubItem,
		...
		gc.setClipping(DPIUtil.scaleDown(new Rectangle(cellRect.left,
				cellRect.top, cellWidth, cellHeight), getZoom()));
	...
	Event sendPaintItemEvent(TableItem_Old item, NMTTCUSTOMDRAW nmcd,
			int column,
			RECT itemRect) {
		int nSavedDC = OS.SaveDC(nmcd.hdc);
	...
	void setBackgroundPixel(int newPixel) {
		...
		/*
		 * Feature in Windows. When the background color is changed, the table
		 * does not redraw until the next WM_PAINT. The fix is to force a
		 * redraw.
		 */
		OS.InvalidateRect(handle, null, true);
	}
	...
	void setBackgroundTransparent(boolean transparent) {
		 ...
		 * Bug in Windows. When the table has the extended style
		 * LVS_EX_FULLROWSELECT and LVM_SETBKCOLOR is used with CLR_NONE to make
		 * the table transparent, Windows draws a black rectangle around the
		 * first column. The fix is clear LVS_EX_FULLROWSELECT.
		 *
		 ...
		 * image and any other custom drawing. The fix is to clear the selected
		 * column.
		 */
		int oldPixel = (int) OS.SendMessage(handle, OS.LVM_GETBKCOLOR, 0, 0);
		if (transparent) {
			if (oldPixel != OS.CLR_NONE) {
				/*
				 * Bug in Windows. When the background color is changed, the
				 * table does not redraw until the next WM_PAINT. The fix is to
				 * force a redraw.
				 */
				OS.SendMessage(handle, OS.LVM_SETBKCOLOR, 0, OS.CLR_NONE);
				...
				if ((sortDirection & (SWT.UP | SWT.DOWN)) != 0) {
					if (sortColumn != null && !sortColumn.isDisposed()) {
						OS.SendMessage(handle, OS.LVM_SETSELECTEDCOLUMN, -1, 0);
						/*
						 * Bug in Windows. When LVM_SETSELECTEDCOLUMN is set,
						 * Windows does not redraw either the new or the
						 * previous selected column. The fix is to force a
						 * redraw.
						 */
						OS.InvalidateRect(handle, null, true);
					}
					...
					if (sortColumn != null && !sortColumn.isDisposed()) {
						...
						if (column != -1) {
							OS.SendMessage(handle, OS.LVM_SETSELECTEDCOLUMN,
									column, 0);
							/*
							 * Bug in Windows. When LVM_SETSELECTEDCOLUMN is
							 * set, Windows does not redraw either the new or
							 * the previous selected column. The fix is to force
							 * a redraw.
							 */
							OS.InvalidateRect(handle, null, true);
						}
	...
	public void setColumnOrder(int[] order) {
		...
		if (reorder) {
			...
			OS.SendMessage(handle, OS.LVM_SETCOLUMNORDERARRAY, order.length,
					order);
			/*
			...
			for (int i = 0; i < columnCount; i++) {
				TableColumn_Old column = newColumns[i];
				if (!column.isDisposed()) {
	...
	void setDeferResize(boolean defer) {
		if (defer) {
			if (resizeCount++ == 0) {
				 ...
				 * to make the background of the table transparent, drawing
				 * becomes slow. The fix is to temporarily clear CLR_NONE when
				 * redraw is turned off.
				 */
				if (hooks(SWT.MeasureItem) || hooks(SWT.EraseItem)
						|| hooks(SWT.PaintItem)) {
			...
			if (--resizeCount == 0) {
				if (hooks(SWT.MeasureItem) || hooks(SWT.EraseItem)
						|| hooks(SWT.PaintItem)) {
					if (--drawCount == 0 /* && OS.IsWindowVisible (handle) */) {
						OS.SendMessage(handle, OS.LVM_SETBKCOLOR, 0,
								OS.CLR_NONE);
						OS.DefWindowProc(handle, OS.WM_SETREDRAW, 1, 0);
	...
	void setCheckboxImageList(int width, int height, boolean fixScroll) {
		...
		if (!OS.IsAppThemed())
			flags |= OS.ILC_MASK;
		long hStateList = OS.ImageList_Create(width, height, flags, count,
				count);
		long hDC = OS.GetDC(handle);
		...
		if (OS.IsAppThemed()) {
			...
			rect.right += width;
			OS.DrawThemeBackground(hTheme, memDC, OS.BP_CHECKBOX,
					OS.CBS_UNCHECKEDDISABLED, rect, null);
			rect.left += width;
			rect.right += width;
			OS.DrawThemeBackground(hTheme, memDC, OS.BP_CHECKBOX,
					OS.CBS_CHECKEDDISABLED, rect, null);
			rect.left += width;
			rect.right += width;
			OS.DrawThemeBackground(hTheme, memDC, OS.BP_CHECKBOX,
					OS.CBS_UNCHECKEDDISABLED, rect, null);
			...
			rect.right += width;
			OS.DrawThemeBackground(hTheme, memDC, OS.BP_CHECKBOX,
					OS.CBS_MIXEDDISABLED, rect, null);
		} else {
			...
			OS.DrawFrameControl(memDC, rect, OS.DFC_BUTTON, OS.DFCS_BUTTONCHECK
					| OS.DFCS_CHECKED | OS.DFCS_INACTIVE | OS.DFCS_FLAT);
			rect.left += width;
			rect.right += width;
			OS.DrawFrameControl(memDC, rect, OS.DFC_BUTTON,
					OS.DFCS_BUTTONCHECK | OS.DFCS_FLAT);
		...
		long hOldStateList = OS.SendMessage(handle, OS.LVM_GETIMAGELIST,
				OS.LVSIL_STATE, 0);
		OS.SendMessage(handle, OS.LVM_SETIMAGELIST, OS.LVSIL_STATE, hStateList);
		if (hOldStateList != 0)
			OS.ImageList_Destroy(hOldStateList);
		/*
		 * Bug in Windows. Setting the LVSIL_STATE state image list when the
		 * table already has a LVSIL_SMALL image list causes pixel corruption of
		 * the images. The fix is to reset the LVSIL_SMALL image list.
		 */
		long hImageList = OS.SendMessage(handle, OS.LVM_GETIMAGELIST,
				OS.LVSIL_SMALL, 0);
	...
	public void setFont(Font font) {
		...
		if (itemHeight != -1) {
			int bits = OS.GetWindowLong(handle, OS.GWL_STYLE);
			OS.SetWindowLong(handle, OS.GWL_STYLE,
					bits | OS.LVS_OWNERDRAWFIXED);
		}
		super.setFont(font);
		if (itemHeight != -1) {
			int bits = OS.GetWindowLong(handle, OS.GWL_STYLE);
	...
	void setForegroundPixel(int pixel) {
		/*
		 * The Windows table control uses CLR_DEFAULT to indicate that it is
		 * using the default foreground color. This is undocumented.
		 */
		if (pixel == -1)
	...
	public void setHeaderVisible(boolean show) {
		 ...
		 * top index causing the table to scroll to the new location.
		 */
	...
	public void setItemCount(int count) {
		...
		if (isVirtual) {
			 ...
			 * specifying LVSICF_NOINVALIDATEALL or LVSICF_NOSCROLL does correct
			 * the problem. The fix is to force a redraw.
			 */
			...
			for (int i = itemCount; i < count; i++) {
				new TableItem_Old(this, SWT.NONE, i, true);
			}
		}
		if (!isVirtual)
			setRedraw(true);
	...
	void setItemHeight(boolean fixScroll) {
		...
		if (itemHeight == -1) {
			/*
			 * Feature in Windows. Windows has no API to restore the defualt
			 * item height for a table. The fix is to use WM_SETFONT which
			 * recomputes and assigns the default item height.
			 */
			long hFont = OS.SendMessage(handle, OS.WM_GETFONT, 0, 0);
			OS.SendMessage(handle, OS.WM_SETFONT, hFont, 0);
		} else {
			/*
			 * Feature in Windows. Window has no API to set the item height for
			 * a table. The fix is to set temporarily set LVS_OWNERDRAWFIXED
			 * then resize the table, causing a WM_MEASUREITEM to be sent, then
			 * clear LVS_OWNERDRAWFIXED.
			 */
			forceResize();
			RECT rect = new RECT();
	...
	public void setRedraw(boolean redraw) {
		checkWidget();
		/*
		 * Feature in Windows. When WM_SETREDRAW is used to turn off drawing in
		 * a widget, it clears the WS_VISIBLE bits and then sets them when
		...
		if (redraw) {
			if (--drawCount == 0) {
				 ...
				 * temporarily unsubclass the window proc so that messages are
				 * dispatched directly to the table.
				 *
				 * NOTE: This is optimization somewhat dangerous because any
				 * operation can occur when redraw is turned off, even
				 * operations where the table must be subclassed in order to
				 * have the correct behavior or work around a Windows bug.
				 *
				 * This code is intentionally commented.
				 */
				// subclass ();
				...
				/*
				 * Bug in Windows. For some reason, when WM_SETREDRAW is used to
				 * turn redraw back on this may result in a WM_SIZE. If the
				 * table column widths are adjusted in WM_SIZE, blank lines may
				 * be inserted at the top of the widget. A call to
				 * LVM_GETTOPINDEX will return a negative number (this is an
				 * impossible result). The fix is to send the resize
				 * notification after the size has been changed in the operating
				 * system.
				 */
				setDeferResize(true);
	...
	void setScrollWidth(int width) {
		if (width != (int) OS.SendMessage(handle, OS.LVM_GETCOLUMNWIDTH, 0,
				0)) {
			/*
			 * Feature in Windows. When LVM_SETCOLUMNWIDTH is sent, Windows
			 * draws right away instead of queuing a WM_PAINT. This can cause
			 * recursive calls when called from paint or from messages that are
			 * retrieving the item data, such as WM_NOTIFY, causing a stack
			 * overflow. The fix is to turn off redraw and queue a repaint,
			 * collapsing the recursive calls.
			 */
	...
	boolean setScrollWidth(TableItem_Old item, boolean force) {
		if (currentItem != null) {
			if (currentItem != item)
				fixScrollWidth = true;
			return false;
		...
		if (columnCount == 0) {
			...
			while (index < itemCount) {
				...
				if (item != null) {
					...
					TableItem_Old tableItem = _getItem(index, false);
					if (tableItem != null) {
						string = tableItem.text;
						imageIndent = Math.max(imageIndent,
								tableItem.imageIndent);
					...
					if (hFont != -1) {
						...
						newWidth = Math.max(newWidth,
								(int) OS.SendMessage(handle,
										OS.LVM_GETSTRINGWIDTH, 0, buffer));
					}
				}
				if (item != null)
					break;
			...
			/*
			 * Bug in Windows. When the width of the first column is small but
			 * not zero, Windows draws '...' outside of the bounds of the text.
			 * This is strange, but only causes problems when the item is
			 * selected. In this case, Windows clears the '...' but doesn't
			 * redraw it when the item is deselected, causing pixel corruption.
			 * The fix is to ensure that the column is at least wide enough to
			 * draw a single space.
			 */
			if (newWidth == 0) {
				char[] buffer = {' ', '\0'};
				newWidth = Math.max(newWidth, (int) OS.SendMessage(handle,
						OS.LVM_GETSTRINGWIDTH, 0, buffer));
			}
			long hStateList = OS.SendMessage(handle, OS.LVM_GETIMAGELIST,
					OS.LVSIL_STATE, 0);
			...
			if (hImageList != 0) {
				 ...
				 * fact that there are none. The fix is to set the image list to
				 * be very small before setting it to NULL. This causes Windows
				 * to reserve the smallest possible space when an image list is
				 * removed. In this case, the scroll width must be one pixel
				 * larger.
				 */
				newWidth++;
			}
			newWidth += INSET * 2 + VISTA_EXTRA;
			int oldWidth = (int) OS.SendMessage(handle, OS.LVM_GETCOLUMNWIDTH,
					0, 0);
	 ...
	 *                </ul>
	 *
	 * @see Table_Old#deselectAll()
	 * @see Table_Old#select(int[])
	 */
	 ...
	 * @since 3.2
	 */
	public void setSelection(TableItem_Old item) {
		checkWidget();
		if (item == null)
			error(SWT.ERROR_NULL_ARGUMENT);
		setSelection(new TableItem_Old[]{item});
	}
	 ...
	 * @see Table_Old#setSelection(int[])
	 */
	public void setSelection(TableItem_Old[] items) {
		checkWidget();
		if (items == null)
			error(SWT.ERROR_NULL_ARGUMENT);
		deselectAll();
	 ...
	 * @since 3.2
	 */
	public void setSortColumn(TableColumn_Old column) {
		checkWidget();
	...
	void setTableEmpty() {
		if (imageList != null) {
			 ...
			 * list by setting it to NULL, the item width and height is not
			 * changed and space is reserved for icons despite the fact that
			 * there are none. The fix is to set the image list to be very small
			 * before setting it to NULL. This causes Windows to reserve the
			 * smallest possible space when an image list is removed.
			 */
	...
	public void setTopIndex(int index) {
		...
		/*
		 * Bug in Windows. For some reason, LVM_SCROLL refuses to scroll a table
		 * vertically when the width and height of the table is smaller than a
		 * certain size. The values that seem to cause the problem are width=68
		 * and height=6 but there is no guarantee that these values cause the
		 * failure on different machines or on different versions of Windows. It
		 * may depend on the font and any number of other factors. For example,
		 * setting the font to anything but the default sometimes fixes the
		 * problem. The fix is to use LVM_GETCOUNTPERPAGE to detect the case
		 * when the number of visible items is zero and use LVM_ENSUREVISIBLE to
		 * scroll the table to make the index visible.
		 */
		...
		if (OS.SendMessage(handle, OS.LVM_GETCOUNTPERPAGE, 0, 0) <= 0) {
			/*
			 * Bug in Windows. For some reason, LVM_ENSUREVISIBLE can scroll one
			 * item more or one item less when there is not enough space to show
			 * a single table item. The fix is to detect the case and call
			 * LVM_ENSUREVISIBLE again with the same arguments. It seems that
			 * once LVM_ENSUREVISIBLE has scrolled into the general area, it is
			 * able to scroll to the exact item.
			 */
	 ...
	 * @since 3.0
	 */
	public void showColumn(TableColumn_Old column) {
		checkWidget();
		if (column == null)
			error(SWT.ERROR_NULL_ARGUMENT);
		if (column.isDisposed())
		...
		if (!(0 <= index && index < columnCount))
			return;
		/*
		 * Feature in Windows. Calling LVM_GETSUBITEMRECT with -1 for the row
		 * number gives the bounds of the item that would be above the first row
		 * in the table. This is undocumented and does not work for the first
		 * column. In this case, to get the bounds of the first column, get the
		 * bounds of the second column and subtract the width of the first. The
		 * left edge of the second column is also used as the right edge of the
		 * first.
		 */
		RECT itemRect = new RECT();
		...
		if (index == 0) {
			...
			int width = (int) OS.SendMessage(handle, OS.LVM_GETCOLUMNWIDTH, 0,
					0);
		...
		/*
		 * Bug in Windows. When a table that is drawing grid lines is slowly
		 * scrolled horizontally to the left, the table does not redraw the
		 * newly exposed vertical grid lines. The fix is to save the old scroll
		 * position, call the window proc, get the new scroll position and
		 * redraw the new area.
		 */
		int oldPos = 0;
		if (_getLinesVisible()) {
	...
	void showItem(int index) {
		if (!painted && hooks(SWT.MeasureItem))
			hitTestSelection(index, 0, 0);
		/*
		 * Bug in Windows. For some reason, when there is insufficient space to
		 * show an item, LVM_ENSUREVISIBLE causes blank lines to be inserted at
		 * the top of the widget. A call to LVM_GETTOPINDEX will return a
	 ...
	 *                </ul>
	 *
	 * @see Table_Old#showSelection()
	 */
	public void showItem(TableItem_Old item) {
		checkWidget();
		if (item == null)
			error(SWT.ERROR_NULL_ARGUMENT);
		if (item.isDisposed())
	 ...
	 *                </ul>
	 *
	 * @see Table_Old#showItem(TableItem_Old)
	 */
	public void showSelection() {
		...
		if (index != -1) {
			 ...
			 *
			 * Make sure above fix is only applied to the active shell, see bug
			 * 450391.
			 */
			if (display.getActiveShell() == getShell()
					&& (style & SWT.NO_SCROLL) == 0
	...
	RECT toolTipInset(RECT rect) {
		...
		OS.SetRect(insetRect, rect.left - 1, rect.top - 1, rect.right + 1,
				rect.bottom + 1);
		return insetRect;
	}
	...
	RECT toolTipRect(RECT rect) {
		...
		OS.SetRect(toolRect, rect.left - 1, rect.top - 1, rect.right + 1,
				rect.bottom + 1);
		return toolRect;
	}
	...
	void update(boolean all) {
		...
		if (fixSubclass) {
			oldTableProc = OS.SetWindowLongPtr(handle, OS.GWLP_WNDPROC,
					TableProc);
			oldHeaderProc = OS.SetWindowLongPtr(hwndHeader, OS.GWLP_WNDPROC,
					HeaderProc);
		}
		super.update(all);
		if (fixSubclass) {
	...
	void updateMenuLocation(Event event) {
		...
		if (focusIndex != -1) {
			TableItem_Old focusItem = getItem(focusIndex);
	...
	void updateOrientation() {
		...
		if (hwndHeader != 0) {
			...
			OS.GetWindowRect(handle, rect);
			int width = rect.right - rect.left, height = rect.bottom - rect.top;
			OS.SetWindowPos(handle, 0, 0, 0, width - 1, height - 1,
					OS.SWP_NOMOVE | OS.SWP_NOZORDER);
			OS.SetWindowPos(handle, 0, 0, 0, width, height,
					OS.SWP_NOMOVE | OS.SWP_NOZORDER);
		}
		if ((style & SWT.CHECK) != 0)
		...
		if (imageList != null) {
			...
			display.releaseImageList(imageList);
			imageList = display.getImageList(style & SWT.RIGHT_TO_LEFT, size.x,
					size.y, getZoom());
			int count = (int) OS.SendMessage(handle, OS.LVM_GETITEMCOUNT, 0, 0);
			for (int i = 0; i < count; i++) {
				TableItem_Old item = _getItem(i, false);
		...
		if (hwndHeader != 0) {
			if (headerImageList != null) {
				...
				display.releaseImageList(headerImageList);
				headerImageList = display.getImageList(
						style & SWT.RIGHT_TO_LEFT, size.x, size.y, getZoom());
				if (columns != null) {
					for (int i = 0; i < columns.length; i++) {
						...
						if (column != null) {
							...
							if (image != null) {
								LVCOLUMN lvColumn = new LVCOLUMN();
								lvColumn.mask = OS.LVCF_FMT;
								OS.SendMessage(hwndHeader, OS.LVM_GETCOLUMN, i,
										lvColumn);
								if ((lvColumn.fmt & OS.LVCFMT_IMAGE) != 0) {
									...
									OS.SendMessage(hwndHeader, OS.LVM_SETCOLUMN,
											i, lvColumn);
								}
							}
						}
	...
	boolean updateTextDirection(int textDirection) {
		if (super.updateTextDirection(textDirection)) {
			if (textDirection == AUTO_TEXT_DIRECTION
					|| (state & HAS_AUTO_DIRECTION) != 0) {
				for (TableItem_Old item : items) {
					if (item != null) {
						item.updateTextDirection(
								textDirection == AUTO_TEXT_DIRECTION
										? AUTO_TEXT_DIRECTION
	...
	int widgetStyle() {
		...
		/*
		 * This code is intentionally commented. In the future, the FLAT bit may
		 * be used to make the header flat and unresponsive to mouse clicks.
		 */
	...
	long windowProc(long hwnd, int msg, long wParam, long lParam) {
		...
		if (hwnd != handle) {
			switch (msg) {
				...
				case OS.WM_MOUSELEAVE : {
					/*
					 * Bug in Windows. On XP, when a tooltip is hidden due to a
					 * time out or mouse press, the tooltip remains active
					 * although no longer visible and won't show again until
					 * another tooltip becomes active. The fix is to reset the
					 * tooltip bounds.
					 */
					updateHeaderToolTips();
					updateHeaderToolTips();
					...
					if (wParam == hwnd) {
						...
						if (hitTest == OS.HTCLIENT) {
							...
							long hwndHeader = OS.SendMessage(handle,
									OS.LVM_GETHEADER, 0, 0);
							int index = (int) OS.SendMessage(hwndHeader,
									OS.HDM_HITTEST, 0, pinfo);
							if (0 <= index && index < columnCount
									&& !columns[index].resizable) {
								if ((pinfo.flags & (OS.HHT_ONDIVIDER
										| OS.HHT_ONDIVOPEN)) != 0) {
									OS.SetCursor(
		...
		if (msg == Display.DI_GETDRAGIMAGE) {
			 ...
			 *
			 * Bug in Windows. For custom draw control the window origin the in
			 * HDC is wrong.
			 *
			 * The fix for both cases is to create the image using
			 * PrintWindow().
			 */
			int topIndex = (int) OS.SendMessage(handle, OS.LVM_GETTOPINDEX, 0,
					0);
			int selection = (int) OS.SendMessage(handle, OS.LVM_GETNEXTITEM,
			...
			TableItem_Old item = _getItem(selection);
			...
			long hRgn = OS.CreateRectRgn(rect.left, rect.top, rect.right,
					rect.bottom);
			while ((selection = (int) OS.SendMessage(handle, OS.LVM_GETNEXTITEM,
					selection, OS.LVNI_SELECTED)) != -1) {
				if (rect.bottom - rect.top > DRAG_IMAGE_SIZE)
					break;
				if (rect.bottom > clientRect.bottom)
					break;
				RECT itemRect = item.getBounds(selection, 0, true, true, true);
				long rectRgn = OS.CreateRectRgn(rect.left, itemRect.top,
			...
			long memDib = OS.CreateDIBSection(0, bmi, OS.DIB_RGB_COLORS, pBits,
					0, 0);
			if (memDib == 0)
				error(SWT.ERROR_NO_HANDLES);
	...
	LRESULT WM_CHAR(long wParam, long lParam) {
		...
		switch ((int) wParam) {
			case ' ' :
				if ((style & SWT.CHECK) != 0) {
					int index = -1;
					while ((index = (int) OS.SendMessage(handle,
							OS.LVM_GETNEXTITEM, index,
							OS.LVNI_SELECTED)) != -1) {
						TableItem_Old item = _getItem(index);
				...
				/*
				 * NOTE: Call the window proc with WM_KEYDOWN rather than
				 * WM_CHAR so that the key that was ignored during WM_KEYDOWN is
				 * processed. This allows the application to cancel an operation
				 * that is normally performed in WM_KEYDOWN from WM_CHAR.
				 */
				long code = callWindowProc(handle, OS.WM_KEYDOWN, wParam,
						lParam);
				return new LRESULT(code);
			case SWT.CR :
				/*
				 * Feature in Windows. Windows sends LVN_ITEMACTIVATE from
				 * WM_KEYDOWN instead of WM_CHAR. This means that application
				 * code that expects to consume the key press and therefore
				 * avoid a SWT.DefaultSelection event will fail. The fix is to
				 * ignore LVN_ITEMACTIVATE when it is caused by WM_KEYDOWN and
				 * send SWT.DefaultSelection from WM_CHAR.
				 */
				int index = (int) OS.SendMessage(handle, OS.LVM_GETNEXTITEM, -1,
						OS.LVNI_FOCUSED);
				if (index != -1) {
	...
	LRESULT WM_CONTEXTMENU(long wParam, long lParam) {
		/*
		 * Feature in Windows. For some reason, when the right mouse button is
		 * pressed over an item, Windows sends a WM_CONTEXTMENU from
		 * WM_RBUTTONDOWN, instead of from WM_RBUTTONUP. This causes two context
		 * menus requests to be sent. The fix is to ignore WM_CONTEXTMENU on
		 * mouse down.
		 *
		 * NOTE: This only happens when dragging is disabled. When the table is
		 * detecting drag, the WM_CONTEXTMENU is not sent WM_RBUTTONUP.
		 */
	...
	LRESULT WM_GETOBJECT(long wParam, long lParam) {
		 ...
		 * because support for checked item accessibility is temporarily
	...
	LRESULT WM_KEYDOWN(long wParam, long lParam) {
		...
		switch ((int) wParam) {
			case OS.VK_SPACE :
				/*
				 * Ensure that the window proc does not process VK_SPACE so that
				 * it can be handled in WM_CHAR. This allows the application to
				 * cancel an operation that is normally performed in WM_KEYDOWN
				 * from WM_CHAR.
				 */
				return LRESULT.ZERO;
			case OS.VK_ADD :
				if (OS.GetKeyState(OS.VK_CONTROL) < 0) {
					...
					if (index != columnCount || hooks(SWT.MeasureItem)) {
						TableColumn_Old[] newColumns = new TableColumn_Old[columnCount];
						System.arraycopy(columns, 0, newColumns, 0,
								columnCount);
						for (TableColumn_Old column : newColumns) {
			...
			case OS.VK_END :
				 ...
				 * so that internal messages are dispatched directly to the
				 * table. If the application expects to see a paint event, the
				 * window proc cannot be unsubclassed or the event will not be
				 * seen.
				 *
				 * NOTE: The header tooltip can subclass the header proc so the
				 * current proc must be restored or header tooltips stop
				 * working.
				 */
				long oldHeaderProc = 0, oldTableProc = 0;
				long hwndHeader = OS.SendMessage(handle, OS.LVM_GETHEADER, 0,
			...
			case OS.VK_UP :
			case OS.VK_DOWN :
				OS.SendMessage(handle, OS.WM_CHANGEUISTATE, OS.UIS_INITIALIZE,
						0);
				break;
		}
		return result;
	...
	LRESULT WM_KILLFOCUS(long wParam, long lParam) {
		...
		/*
		 * Bug in Windows. When focus is lost, Windows does not redraw the
		 * selection properly, leaving the image and check box appearing
		 * selected. The fix is to redraw the table.
		 */
	...
	LRESULT WM_LBUTTONDBLCLK(long wParam, long lParam) {
		...
		if ((style & SWT.CHECK) != 0) {
			/*
			 * Note that when the table has LVS_EX_FULLROWSELECT and the user
			 * clicks anywhere on a row except on the check box, all of the bits
			 * are set. The hit test flags are LVHT_ONITEM. This means that a
			 * bit test for LVHT_ONITEMSTATEICON is not the correct way to
			 * determine that the user has selected the check box, equality is
			 * needed.
			 */
	...
	LRESULT WM_LBUTTONDOWN(long wParam, long lParam) {
		 ...
		 * mouse processing for the widget. The fix is to avoid the automatic
		 * mouse capture.
		 */
	...
	LRESULT WM_MOUSEHOVER(long wParam, long lParam) {
		/*
		 * Feature in Windows. Despite the fact that hot tracking is not
		 * enabled, the hot tracking code in WM_MOUSEHOVER is executed causing
		 * the item under the cursor to be selected. The fix is to avoid calling
		 * the window proc.
		 */
	...
	LRESULT WM_SETFOCUS(long wParam, long lParam) {
		...
		/*
		 * Bug in Windows. When focus is gained after the selection has been
		 * changed using LVM_SETITEMSTATE, Windows redraws the selected text but
		 * does not redraw the image or the check box, leaving them appearing
		 * unselected. The fix is to redraw the table.
		 */
		if (imageList != null || (style & SWT.CHECK) != 0) {
			OS.InvalidateRect(handle, null, false);
		}
		...
		/*
		 * Bug in Windows. For some reason, the table does not set the default
		 * focus rectangle to be the first item in the table when it gets focus
		 * and there is no selected item. The fix to make the first item be the
		 * focus item.
		 */
		int count = (int) OS.SendMessage(handle, OS.LVM_GETITEMCOUNT, 0, 0);
		if (count == 0)
	...
	LRESULT WM_SETFONT(long wParam, long lParam) {
		...
		/*
		 * Bug in Windows. When a header has a sort indicator triangle, Windows
		 * resizes the indicator based on the size of the n-1th font. The fix is
		 * to always make the n-1th font be the default. This makes the sort
		 * indicator always be the default size.
		 *
		 * NOTE: The table window proc sets the actual font in the header so
		 * that all that is necessary here is to set the default first.
		 */
	...
	LRESULT WM_SETREDRAW(long wParam, long lParam) {
		...
		if (result != null)
			return result;
		/*
		 * Feature in Windows. When LVM_SETBKCOLOR is used with CLR_NONE to make
		 * the background of the table transparent, drawing becomes slow. The
		 * fix is to temporarily clear CLR_NONE when redraw is turned off.
		 */
		if (wParam == 1) {
			if ((int) OS.SendMessage(handle, OS.LVM_GETBKCOLOR, 0,
					0) != OS.CLR_NONE) {
	...
	LRESULT WM_SYSCOLORCHANGE(long wParam, long lParam) {
		...
		if (findBackgroundControl() == null) {
			...
			int oldPixel = (int) OS.SendMessage(handle, OS.LVM_GETBKCOLOR, 0,
					0);
			if (oldPixel != OS.CLR_NONE) {
				if (findImageControl() == null) {
					if ((style & SWT.CHECK) != 0)
	...
	LRESULT WM_HSCROLL(long wParam, long lParam) {
		...
		/*
		 * Feature in Windows. When there are many columns in a table, scrolling
		 * performance can be improved by unsubclassing the window proc so that
		 * internal messages are dispatched directly to the table. If the
		 * application expects to see a paint event or has a child whose font,
		 * foreground or background color might be needed, the window proc
		 * cannot be unsubclassed
		...
		/*
		 * Feature in Windows. For some reason, when the table window proc
		 * processes WM_HSCROLL or WM_VSCROLL when there are many columns in the
		 * table, scrolling is slow and the table does not keep up with the
		 * position of the scroll bar. The fix is to turn off redraw, scroll,
		 * turn redraw back on and redraw the entire table. Strangly, redrawing
		 * the entire table is faster.
		 */
		boolean fixScroll = false;
		if (OS.LOWORD(wParam) != OS.SB_ENDSCROLL) {
			if (columnCount > H_SCROLL_LIMIT) {
				int rowCount = (int) OS.SendMessage(handle,
						OS.LVM_GETCOUNTPERPAGE, 0, 0);
		...
		if (fixScroll) {
			...
			OS.RedrawWindow(handle, null, 0, flags);
			/*
			 * Feature in Windows. On Vista only, it is faster to compute and
			 * answer the data for the visible columns of a table when
			 * scrolling, rather than just return the data for each column when
			 * asked.
			 */
			...
			for (int i = 0; i < columnCount; i++) {
				...
				if (OS.SendMessage(handle, OS.LVM_GETSUBITEMRECT, 0,
						headerRect) != 0) {
	...
	LRESULT WM_VSCROLL(long wParam, long lParam) {
		...
		/*
		 * Bug in Windows. When a table is drawing grid lines and the user
		 * scrolls vertically up or down by a line or a page, the table does not
		 * redraw the grid lines for newly exposed items. The fix is to
		...
		if (_getLinesVisible()) {
			...
			switch (code) {
				...
				case OS.SB_LINEUP :
					...
					long empty = OS.SendMessage(handle,
							OS.LVM_APPROXIMATEVIEWRECT, 0, 0);
					long oneItem = OS.SendMessage(handle,
							OS.LVM_APPROXIMATEVIEWRECT, 1, 0);
					int itemHeight = OS.HIWORD(oneItem) - OS.HIWORD(empty);
					if (code == OS.SB_LINEDOWN) {
						clientRect.top = clientRect.bottom - itemHeight
								- GRID_WIDTH;
					} else {
						clientRect.bottom = clientRect.top + itemHeight
								+ GRID_WIDTH;
					}
	...
	LRESULT wmNotifyChild(NMHDR hdr, long wParam, long lParam) {
		switch (hdr.code) {
			...
			case OS.LVN_ODSTATECHANGED : {
				if ((style & SWT.VIRTUAL) != 0) {
					if (!ignoreSelect) {
						...
						OS.MoveMemory(lpStateChange, lParam,
								NMLVODSTATECHANGE.sizeof);
						boolean oldSelected = (lpStateChange.uOldState
								& OS.LVIS_SELECTED) != 0;
						boolean newSelected = (lpStateChange.uNewState
								& OS.LVIS_SELECTED) != 0;
			...
			case OS.LVN_GETDISPINFO : {
				...
				/*
				 * Feature in Windows. When a new table item is inserted using
				 * LVM_INSERTITEM in a table that is transparent (ie.
				 * LVM_SETBKCOLOR has been called with CLR_NONE), TVM_INSERTITEM
				 * calls LVN_GETDISPINFO before the item has been added to the
				 * array. The fix is to check for null.
				 *
				 * NOTE: Force the item to be created if it does not exist.
				 */
				TableItem_Old item = _getItem(plvfi.iItem);
				if (item == null)
					break;

				/*
				 * Feature in Windows. On Vista, the list view expects the item
				 * array to be up to date when a LVM_DELETEITEM message is being
				 * processed.
				 *
				 * Also, when the table is virtual, do not allow the application
				 * to provide data for a new item that becomes visible until the
				 * item has been removed from the items array. Because arbitrary
				 * application code can run during the callback, the items array
				 * might be accessed in an inconsistent state.
				 *
				 * On both cases, Rather than answering the data right away,
				 * queue a redraw for later.
				 */
				if (ignoreShrink) {
					/*
					 * Feature in Windows Vista and newer. Using LVM_REDRAWITEMS
					 * causes LVN_GETDISPINFO to be sent before the method
					 * returns. For this reason, LVM_REDRAWITEMS can never be
					 * used from a LVN_GETDISPINFO handler. The fix is to
				 ...
				 * tables to indicate that Windows has asked at least once for a
				 * table item.
				 */
				if (!item.cached) {
					if ((style & SWT.VIRTUAL) != 0) {
						...
						if (!checkData(item, lastIndexOf, false))
							break;
						TableItem_Old newItem = fixScrollWidth ? null : item;
				...
				if ((plvfi.mask & OS.LVIF_TEXT) != 0) {
					...
					if (string != null) {
						 ...
						 * selection draws using the bounds of the previous
						 * text. The fix is to use a space rather than an empty
						 * string when anything but a tool tip is requested (to
						 * avoid a tool tip that is a single space).
						 *
						 ...
						 * is set.
						 */
						int length = Math.min(string.length(),
								Math.max(0, plvfi.cchTextMax - 1));
						if (!tipRequested && plvfi.iSubItem == 0
								&& length == 0) {
							string = " "; //$NON-NLS-1$
							length = 1;
						}
						...
						if (buffer == null
								|| plvfi.cchTextMax > buffer.length) {
							buffer = display.tableBuffer = new char[plvfi.cchTextMax];
						}
						string.getChars(0, length, buffer, 0);
						if (tipRequested) {
							/*
							 * Bug in Windows. The tooltip is only displayed up
							 * to the first line delimiter. The fix is to remove
							 * all line delimiter characters.
							 */
							int shift = 0;
			...
			case OS.NM_CUSTOMDRAW : {
				...
				if (!customDraw && findImageControl() == null) {
					 ...
					 * with a gray background but does not gray the text. The
					 * fix is to explicitly gray the text using Custom Draw.
					 */
					if (OS.IsWindowEnabled(handle)) {
						/*
						 * Feature in Windows. On Vista using the explorer
						 * theme, Windows draws a vertical line to separate
						 * columns. When there is only a single column, the line
						 * looks strange. The fix is to draw the background
						 * using custom draw.
						 */
						if (!explorerTheme || columnCount != 0)
							break;
			...
			case OS.LVN_COLUMNCLICK : {
				...
				TableColumn_Old column = columns[pnmlv.iSubItem];
				if (column != null) {
				...
				if (!ignoreSelect) {
					...
					if ((pnmlv.uChanged & OS.LVIF_STATE) != 0) {
						if (pnmlv.iItem == -1) {
							...
							boolean oldSelected = (pnmlv.uOldState
									& OS.LVIS_SELECTED) != 0;
							boolean newSelected = (pnmlv.uNewState
									& OS.LVIS_SELECTED) != 0;
							if (oldSelected != newSelected)
								wasSelected = true;
						}
					...
					if (count != 0) {
						...
						if (pnmlv.iItem != -1) {
							...
							OS.SendMessage(handle, OS.LVM_GETITEMRECT,
									pnmlv.iItem, itemRect);
							ignoreCustomDraw = false;
							RECT headerRect = new RECT();
							int index = (int) OS.SendMessage(hwndHeader,
									OS.HDM_ORDERTOINDEX, count - 1, 0);
							OS.SendMessage(hwndHeader, OS.HDM_GETITEMRECT,
									index, headerRect);
							OS.MapWindowPoints(hwndHeader, handle, headerRect,
									2);
	...
	LRESULT wmNotifyHeader(NMHDR hdr, long wParam, long lParam) {
		/*
		 * Feature in Windows. On NT, the automatically created header control
		 * is created as a UNICODE window, not an ANSI window despite the fact
		 * that the parent is created as an ANSI window. This means that it
		 ...
		 * necessary. Despite this, Windows sends different messages. The fix is
		 * to look for both messages, despite the platform. This works because
		 * only one will be sent on either platform, never both.
		 */
		switch (hdr.code) {
			...
			case OS.HDN_DIVIDERDBLCLICK : {
				...
				TableColumn_Old column = columns[phdn.iItem];
			...
			case OS.NM_CUSTOMDRAW : {
				...
				switch (nmcd.dwDrawStage) {
					case OS.CDDS_PREPAINT : {
						/*
						 * Drawing here will be deleted by further drawing
						 * steps, even with OS.CDRF_SKIPDEFAULT. Changing the
						 * TextColor and returning OS.CDRF_NEWFONT has no
						 * effect.
						 */
						return new LRESULT(customHeaderDrawing()
								? OS.CDRF_NOTIFYITEMDRAW
										| OS.CDRF_NOTIFYPOSTPAINT
								: OS.CDRF_DODEFAULT);
					}
					case OS.CDDS_ITEMPREPAINT : {
						// draw background
						...
						/*
						 * Don't change the header background color for set
						 * selected column, similar to Windows 10 which itself
						 * does not use any different color for sort header. For
						 * more details refer bug 536020
						 */
						// else if (columns[(int) nmcd.dwItemSpec] == sortColumn
						// && sortDirection != SWT.NONE) {
						...
						return new LRESULT(OS.CDRF_SKIPDEFAULT); // if we got
																	// here, we
																	// will
																	// paint
																	// everything
					...
					case OS.CDDS_POSTPAINT : {
						...
						for (int i = 0; i < columnCount; i++) {
							rects[i] = new RECT();
							OS.SendMessage(hwndHeader, OS.HDM_GETITEMRECT, i,
									rects[i]);
							...
							if (columns[i] == sortColumn
									&& sortDirection != SWT.NONE) {
								// the display.getSortImage looks terrible after
								// scaling up.
								long pen = OS.CreatePen(OS.PS_SOLID, 1,
										getHeaderForegroundPixel());
								long oldPen = OS.SelectObject(nmcd.hdc, pen);
								int center = rects[i].left
										+ (rects[i].right - rects[i].left) / 2;
								/*
								 * Sort indicator size needs to scale as per the
								 * Native Windows OS DPI level when header is
								 * custom drawn. For more details refer bug
								 * 537097.
								 */
								int leg = DPIUtil.scaleUp(3, nativeZoom);
								if (sortDirection == SWT.UP) {
									OS.Polyline(nmcd.hdc,
											new int[]{center - leg, 1 + leg,
													center + 1, 0},
											2);
									OS.Polyline(nmcd.hdc,
											new int[]{center + leg, 1 + leg,
							...
							int alignmentCorrection = _getLinesVisible()
									? 0
									: 1;

							/*
							 * Windows 7 and 10 always draw a nearly invisible
							 * vertical line between the columns, even if lines
							 * are disabled. This line uses no fixed color
							 * constant, but calculates it from the background
							 * color. The method getSlightlyDifferentColor gives
							 * us a color, that is near enough to the windows
							 * algorithm.
							 *
							 * NOTE: This code has no effect since Bug 517003,
							 * because next OS.Polyline() draws over the same
							 * coords.
							 *
							 * long pen = OS.CreatePen (OS.PS_SOLID,
							 * getGridLineWidthInPixels(),
							 * getSlightlyDifferentColor(
							 * getHeaderBackgroundPixel())); long oldPen =
							 * OS.SelectObject (nmcd.hdc, pen);
							 * OS.Polyline(nmcd.hdc, new int[]
							 * {rects[i].right-alignmentCorrection,
							 * rects[i].top, rects[i].right-alignmentCorrection,
							 * rects[i].bottom}, 2); OS.SelectObject (nmcd.hdc,
							 * oldPen); OS.DeleteObject (pen);
							 */

							int lineColor = (display.tableHeaderLinePixel != -1)
									? display.tableHeaderLinePixel
									: OS.GetSysColor(OS.COLOR_3DFACE);
							long pen = OS.CreatePen(OS.PS_SOLID,
									getGridLineWidthInPixels(), lineColor);
							long oldPen = OS.SelectObject(nmcd.hdc, pen);
							/*
							 * To differentiate headers, always draw header
							 * column separator.
							 */
							OS.Polyline(nmcd.hdc, new int[]{
									rects[i].right - alignmentCorrection,
									...
									rects[i].bottom}, 2);
							/*
							 * To differentiate header & content area, always
							 * draw the line separator between header & first
							 * row.
							 */
							if (i == 0)
								OS.Polyline(nmcd.hdc,
										new int[]{nmcd.left, nmcd.bottom - 1,
												nmcd.right + 1,
												nmcd.bottom - 1},
										2);
							OS.SelectObject(nmcd.hdc, oldPen);
							...
							if (headerItemDragging
									&& highlightedHeaderDividerX == -1) {
								int distanceToLeftBorder = cursorPos.x
										- rects[i].left;
								int distanceToRightBorder = rects[i].right
										- cursorPos.x;
								if (distanceToLeftBorder >= 0
										&& distanceToRightBorder >= 0) {
									// the cursor is in the current rectangle
									highlightedHeaderDividerX = distanceToLeftBorder <= distanceToRightBorder
											? rects[i].left - 1
											: rects[i].right;
								}
							}
							...
							if (columns[i].image != null) {
								...
								GC gc = createNewGC(nmcd.hdc, data);
								int y = Math.max(0,
										(nmcd.bottom - columns[i].image
												.getBoundsInPixels().height)
												/ 2);
								int zoom = getZoom();
								gc.drawImage(columns[i].image,
										DPIUtil.scaleDown(x, zoom),
										DPIUtil.scaleDown(y, zoom));
								x += columns[i].image.getBoundsInPixels().width
										+ 12;
								gc.dispose();
							}

							if (columns[i].text != null) {
								int flags = OS.DT_NOPREFIX | OS.DT_SINGLELINE
										| OS.DT_VCENTER;
						...
						if (lastColumnRight < nmcd.right) {
							...
							OS.SetRect(rect, lastColumnRight, nmcd.top,
									nmcd.right, nmcd.bottom - 1);
							long brush = OS.CreateSolidBrush(
									getHeaderBackgroundPixel());
							OS.FillRect(nmcd.hdc, rect, brush);
							OS.DeleteObject(brush);
						}

						// always draw the highlighted border at the end, to
						// avoid overdrawing by other borders.
						if (highlightedHeaderDividerX != -1) {
							long pen = OS.CreatePen(OS.PS_SOLID, 4,
									OS.GetSysColor(OS.COLOR_HIGHLIGHT));
							long oldPen = OS.SelectObject(nmcd.hdc, pen);
							OS.Polyline(nmcd.hdc,
									new int[]{highlightedHeaderDividerX,
											nmcd.top, highlightedHeaderDividerX,
											nmcd.bottom},
									2);
							OS.SelectObject(nmcd.hdc, oldPen);
							OS.DeleteObject(pen);
			...
			case OS.HDN_ENDDRAG : {
				...
				if (phdn.iItem != -1 && phdn.pitem != 0) {
					...
					if ((pitem.mask & OS.HDI_ORDER) != 0
						...
						ignoreColumnMove = false;
						for (int i = start; i <= end; i++) {
							TableColumn_Old column = columns[order[i]];
							if (!column.isDisposed()) {
			...
			case OS.HDN_ITEMCHANGED : {
				/*
				 * Bug in Windows. When a table has the LVS_EX_GRIDLINES
				 * extended style and the user drags any column over the first
				 * column in the table, making the size become zero, when the
				 * user drags a column such that the size of the first column
				 * becomes non-zero, the grid lines are not redrawn. The fix is
				 * to detect the case and force a redraw of the first column.
				 */
				int width = (int) OS.SendMessage(handle, OS.LVM_GETCOLUMNWIDTH,
						0, 0);
					...
					if (phdn.pitem != 0) {
						...
						if ((pitem.mask & OS.HDI_WIDTH) != 0) {
							...
							if (column != null) {
								 ...
								 * application code could have disposed the
								 * column in the move event. If this happens,
								 * process the move event for those columns that
								...
								int[] order = new int[columnCount];
								OS.SendMessage(handle,
										OS.LVM_GETCOLUMNORDERARRAY, columnCount,
										order);
								boolean moved = false;
								for (int i = 0; i < columnCount; i++) {
									TableColumn_Old nextColumn = newColumns[order[i]];
		...
		switch (hdr.code) {
			...
			case OS.TTN_SHOW : {
				...
				if (hdr.code != OS.TTN_SHOW)
					tipRequested = true;
				long code = callWindowProc(handle, OS.WM_NOTIFY, wParam,
						lParam);
				if (hdr.code != OS.TTN_SHOW)
					tipRequested = false;
				if (toolTipText != null)
				...
				if (isCustomToolTip()) {
					 ...
					 * hittest a point that is above the table, instead of
					 * returning -1 to indicate that the hittest failed, a
					 * negative index is returned. The fix is to consider any
					 * value that is negative a failure.
					 */
					if (OS.SendMessage(handle, OS.LVM_SUBITEMHITTEST, 0,
							pinfo) >= 0) {
						TableItem_Old item = _getItem(pinfo.iItem);
						long hDC = OS.GetDC(handle);
						long oldFont = 0, newFont = OS.SendMessage(handle,
								OS.WM_GETFONT, 0, 0);
						if (newFont != 0)
							oldFont = OS.SelectObject(hDC, newFont);
						...
						if (hFont != -1)
							hFont = OS.SelectObject(hDC, hFont);
						Event event = sendMeasureItemEvent(item, pinfo.iItem,
								pinfo.iSubItem, hDC);
						if (!isDisposed() && !item.isDisposed()) {
							RECT itemRect = new RECT();
							Rectangle boundsInPixels = DPIUtil
									.scaleUp(event.getBounds(), getZoom());
							OS.SetRect(itemRect, boundsInPixels.x,
									boundsInPixels.y,
									boundsInPixels.x + boundsInPixels.width,
							...
							if (hdr.code == OS.TTN_SHOW) {
								...
								OS.SetWindowPos(hwndToolTip, 0, toolRect.left,
										toolRect.top, width, height, flags);
							} else {
								NMTTDISPINFO lpnmtdi = new NMTTDISPINFO();
								OS.MoveMemory(lpnmtdi, lParam,
										NMTTDISPINFO.sizeof);
								if (lpnmtdi.lpszText != 0) {
									OS.MoveMemory(lpnmtdi.lpszText, new char[1],
											2);
									OS.MoveMemory(lParam, lpnmtdi,
											NMTTDISPINFO.sizeof);
								}
								RECT cellRect = item.getBounds(pinfo.iItem,
										pinfo.iSubItem, true, true, true, true,
										hDC);
								RECT clientRect = new RECT();
								OS.GetClientRect(handle, clientRect);
								if (itemRect.right > cellRect.right
									...
									// } else {
									// String [] strings = item.strings;
									// if (strings != null) string = strings
									// [pinfo.iSubItem];
									// }
									if (string != null) {
										...
										char[] chars = new char[string.length()
												+ 1];
										string.getChars(0, string.length(),
												chars, 0);
		...
		switch (nmcd.dwDrawStage) {
			case OS.CDDS_PREPAINT : {
				if (isCustomToolTip()) {
					...
					return new LRESULT(
							OS.CDRF_NOTIFYPOSTPAINT | OS.CDRF_NEWFONT);
				}
				break;
			}
			case OS.CDDS_POSTPAINT : {
				...
				if (OS.SendMessage(handle, OS.LVM_SUBITEMHITTEST, 0,
					...
					RECT cellRect = item.getBounds(pinfo.iItem, pinfo.iSubItem,
							true, true, false, false, hDC);
					if (hooks(SWT.EraseItem)) {
						Event event = sendEraseItemEvent(item, nmcd,
								pinfo.iSubItem, cellRect);
						if (isDisposed() || item.isDisposed())
							break;
						if (event.doit) {
							drawForeground = (event.detail
									& SWT.FOREGROUND) != 0;
						} else {
					...
					if (drawForeground) {
						int nSavedDC = OS.SaveDC(nmcd.hdc);
						int gridWidth = getLinesVisible()
								? Table.GRID_WIDTH
								: 0;
						RECT insetRect = toolTipInset(cellRect);
						OS.SetWindowOrgEx(nmcd.hdc, insetRect.left,
								insetRect.top, null);
						GCData data = new GCData();
						data.device = display;
						data.foreground = OS.GetTextColor(nmcd.hdc);
						...
						if (image != null) {
							...
							RECT imageRect = item.getBounds(pinfo.iItem,
									pinfo.iSubItem, false, true, false, false,
									hDC);
							Point size = imageList == null
									? new Point(rect.width, rect.height)
									: imageList.getImageSize();
							int y = imageRect.top + Math.max(0,
									(imageRect.bottom - imageRect.top - size.y)
											/ 2);
							int zoom = getZoom();
							rect = DPIUtil.scaleDown(rect, zoom);
							gc.drawImage(image, rect.x, rect.y, rect.width,
									rect.height, DPIUtil.scaleDown(x, zoom),
									DPIUtil.scaleDown(y, zoom),
									DPIUtil.scaleDown(size.x, zoom),
									DPIUtil.scaleDown(size.y, zoom));
							x += size.x + INSET
									+ (pinfo.iSubItem == 0 ? -2 : 4);
						} else {
							x += INSET + 2;
						}
						...
						if (string != null) {
							int flags = OS.DT_NOPREFIX | OS.DT_SINGLELINE
									| OS.DT_VCENTER;
							TableColumn_Old column = columns != null
									? columns[pinfo.iSubItem]
									: null;
							if (column != null) {
					...
					if (hooks(SWT.PaintItem)) {
						...
						sendPaintItemEvent(item, nmcd, pinfo.iSubItem,
								itemRect);
					}
					OS.SelectObject(hDC, oldFont);
					OS.ReleaseDC(handle, hDC);
	...
	private static void handleDPIChange(Widget widget, int newZoom,
			float scalingFactor) {
		if (!(widget instanceof Table_Old table)) {
			return;
		}
		...
		if (table.getColumns().length == 0) {
			scrollWidth = Math.round(
					OS.SendMessage(table.handle, OS.LVM_GETCOLUMNWIDTH, 0, 0)
							* scalingFactor);
		}
		...
		for (TableItem_Old item : table.getItems()) {
			DPIZoomChangeRegistry.applyChange(item, newZoom, scalingFactor);
		}
		for (TableColumn_Old TableColumn_Old : table.getColumns()) {
			DPIZoomChangeRegistry.applyChange(TableColumn_Old, newZoom,
					scalingFactor);
		}
