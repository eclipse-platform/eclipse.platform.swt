 ...
 * @since 3.0
 */
public final class TextLayout_Old extends Resource {
	Font font;
	...
	/**
	 * Runs over a certain length (32000 characters / 65536 Glyphs / 32768
	 * pixels - these numbers come from WinAPI docs + analysis done in Bug 23406
	 * Comment 31) will fail to render in ScriptTextOut, ScriptShape,
	 * ScriptPlace so such long runs need to be split into shorter runs. Because
	 * it is expensive to keep testing (with Script*) to maximize the length we
	 * use this heuristic to minimize the length. However splitting the runs
	 * into too short pieces affects performance, so this is a balance.
	 */
	static final int MAX_RUN_LENGTH = 32000;
	/**
	 * When splitting a run (see {@link #MAX_RUN_LENGTH}) the run needs to split
	 * in a way that does not affect the display of the glyphs, so it is
	 * important to not split the run in the middle of a glyph. We use the same
	 * info to find where we can wrap text to find where we can break the runs
	 * (ScriptBreak's info). This setting limits how far back from
	 * {@link #MAX_RUN_LENGTH} the code will search for a break before forcing a
	 * break at {@link #MAX_RUN_LENGTH}.
	 */
	...
	private static class MetricsAdapter {
		...
		public int calcYAdjust(TEXTMETRIC realMetricsInPixels) {
			...
			return (wantMetricsInPixels.tmAscent
					- realMetricsInPixels.tmAscent);
		}
		...
		public boolean GetTextMetrics(long hdc, TEXTMETRIC lptm) {
			// Even in fixed mode, still call original to get `tmAveCharWidth`
			// etc.
		...
		public int GetOutlineTextMetrics(long hdc, int cbData,
		...
		public int ScriptTextOut(long hdc, long psc, int x, int y,
				int fuOptions, RECT lprc, SCRIPT_ANALYSIS psa, long pwcReserved,
				int iReserved, long pwGlyphs, int cGlyphs, long piAdvance,
				long piJustify, long pGoffset) {
			if (isFixedMetrics()) {
				final int yAdjust = calcYAdjust(hdc);
				y += yAdjust;
			}

			return OS.ScriptTextOut(hdc, psc, x, y, fuOptions, lprc, psa,
					pwcReserved, iReserved, pwGlyphs, cGlyphs, piAdvance,
					piJustify, pGoffset);
		}
	}
	 ...
	 * @see #dispose()
	 */
	public TextLayout_Old(Device device) {
		super(device);
		...
		long[] ppv = new long[1];
		OS.OleInitialize(0);
		if (COM.CoCreateInstance(COM.CLSID_CMultiLanguage, 0,
				COM.CLSCTX_INPROC_SERVER, COM.IID_IMLangFontLink2,
				ppv) == OS.S_OK) {
	...
	RECT addClipRect(StyleItem run, RECT clipRect, RECT rect,
	...
	void breakRun(StyleItem run) {
		...
		run.psla = OS.HeapAlloc(hHeap, OS.HEAP_ZERO_MEMORY,
				SCRIPT_LOGATTR.sizeof * chars.length);
		if (run.psla == 0)
	...
	/*
	 * Compute the runs: itemize, shape, place, and reorder the runs. Break
	 * paragraphs into lines, wraps the text, and initialize caches.
	 */
	void computeRuns(GC gc) {
		System.out.println("computeRuns");
		try {
			...
			for (int i = 0; i < allRuns.length - 1; i++) {
				...
				if (tabsInPixels != null && run.tab) {
					...
					if (j == tabsLength) {
						...
						int lastTabWidth = tabsLength > 1
								? tabsInPixels[tabsLength - 1]
										- tabsInPixels[tabsLength - 2]
								: tabsInPixels[0];
						if (lastTabWidth > 0) {
					...
					/*
					 * This block adjusts the indentation after merged tabs
					 * stops. The extra tabs are removed in merge.
					 */
					...
					if (length > 1) {
						...
						if (stop < tabsLength) {
							...
							if (j < tabsLength) {
								run.width += tabsInPixels[tabsLength - 1]
										- tabsInPixels[j];
				...
				if (wrapWidth != -1 && lineWidth + run.width > wrapWidthInPixels
					...
					if (run.style != null && run.style.metrics != null) {
						...
						OS.ScriptGetLogicalWidths(run.analysis, run.length,
					...
					while (i >= lineStart) {
						breakRun(run);
						while (start >= 0) {
							OS.MoveMemory(logAttr,
									run.psla + (start * SCRIPT_LOGATTR.sizeof),
						...
						/*
						 * Bug in Windows. For some reason Uniscribe sets the
						 * fSoftBreak flag for the first letter after a letter
						 * with an accent. This cause a break line to be set in
						 * the middle of a word. The fix is to detect the case
						 * and ignore fSoftBreak forcing the algorithm keep
						 * searching.
						 */
						if (start == 0 && i != lineStart && !run.tab) {
							if (logAttr.fSoftBreak && !logAttr.fWhiteSpace) {
								OS.MoveMemory(properties,
										device.scripts[run.analysis.eScript],
										SCRIPT_PROPERTIES.sizeof);
								int langID = properties.langid;
								StyleItem pRun = allRuns[i - 1];
								OS.MoveMemory(properties,
										device.scripts[pRun.analysis.eScript],
										SCRIPT_PROPERTIES.sizeof);
								if (properties.langid == langID
										|| langID == OS.LANG_NEUTRAL
										|| properties.langid == OS.LANG_NEUTRAL) {
									breakRun(pRun);
									OS.MoveMemory(logAttr,
											pRun.psla + ((pRun.length - 1)
													* SCRIPT_LOGATTR.sizeof),
											SCRIPT_LOGATTR.sizeof);
					...
					boolean wrapEntireRun = start == 0 && i != lineStart
							&& !run.tab;
					if (wrapEntireRun) {
						breakRun(run);
						OS.MoveMemory(logAttr,
					...
					if (wrapEntireRun) {
						...
						/*
						 * No soft-break or line-feed found. Avoid breaking a
						 * run at the first character (firstStart == 0) unless
						 * it's the only run available (firstIndice ==
						 * lineStart). See bug 408833.
						 */
					...
					if (0 < start && start < run.length) {
						...
						StyleItem[] newAllRuns = new StyleItem[allRuns.length
								+ 1];
						System.arraycopy(allRuns, 0, newAllRuns, 0, i + 1);
						System.arraycopy(allRuns, i + 1, newAllRuns, i + 2,
								allRuns.length - i - 1);
						allRuns = newAllRuns;
						allRuns[i + 1] = newRun;
					}
			...
			for (int i = 0; i < allRuns.length; i++) {
				...
				descentInPoints = Math.max(descentInPoints,
						run.descentInPoints);
				if (run.lineBreak || i == allRuns.length - 1) {
					 ...
					 */
					if (lineRunCount == 1
							&& (i == allRuns.length - 1 || !run.softBreak)) {
						TEXTMETRIC lptm = new TEXTMETRIC();
						OS.SelectObject(srcHdc, getItemFont(run, gc));
						metricsAdapter.GetTextMetrics(srcHdc, lptm);
						run.ascentInPoints = DPIUtil.scaleDown(getDevice(),
								lptm.tmAscent, getZoom(gc));
						run.descentInPoints = DPIUtil.scaleDown(getDevice(),
								lptm.tmDescent, getZoom(gc));
						ascentInPoints = Math.max(ascentInPoints,
								run.ascentInPoints);
						descentInPoints = Math.max(descentInPoints,
								run.descentInPoints);
					}
					...
					if (justify && wrapWidth != -1 && run.softBreak
							&& lineWidth > 0) {
						...
						if (line == 0) {
							...
							StyleItem previousRun = previousLine[previousLine.length
									- 1];
							if (previousRun.lineBreak
									&& !previousRun.softBreak) {
								lineIndent = indentInPixels;
							}
						}
						...
						for (StyleItem item : runs[line]) {
							int iDx = item.width * wrapWidthInPixels
									/ lineWidth;
							if (iDx != item.width) {
								item.justify = OS.HeapAlloc(hHeap,
										OS.HEAP_ZERO_MEMORY,
										item.glyphCount * 4);
								if (item.justify == 0)
									SWT.error(SWT.ERROR_NO_HANDLES);
								OS.ScriptJustify(item.visAttrs, item.advances,
					...
					line++;
					lineY[line] = lineY[line - 1] + ascentInPoints
							+ descentInPoints + lineSpacingInPoints;
			...
			System.out.println("computeRunsResult: lineWidths"
					+ Arrays.toString(lineWidthInPixels));
			System.out.println("computeRunsResult: lineOffsets"
					+ Arrays.toString(lineOffset));
		}
	}
	...
	long createGdipBrush(int pixel, int alpha) {
		int argb = ((alpha & 0xFF) << 24) | ((pixel >> 16) & 0xFF)
	...
	public void draw(GC gc, int x, int y) {
		checkLayout();
		drawInPixels(gc, DPIUtil.scaleUp(getDevice(), x, getZoom(gc)),
				DPIUtil.scaleUp(getDevice(), y, getZoom(gc)));
	}
	...
	public void draw(GC gc, int x, int y, int selectionStart, int selectionEnd,
		...
		checkLayout();
		drawInPixels(gc, DPIUtil.scaleUp(getDevice(), x, getZoom(gc)),
				DPIUtil.scaleUp(getDevice(), y, getZoom(gc)), selectionStart,
				selectionEnd, selectionForeground, selectionBackground);
	}

	void drawInPixels(GC gc, int x, int y, int selectionStart, int selectionEnd,
			Color selectionForeground, Color selectionBackground) {
		drawInPixels(gc, x, y, selectionStart, selectionEnd,
				selectionForeground, selectionBackground, 0);
	}
	...
	void drawInPixels(GC gc, int x, int y, int selectionStart, int selectionEnd,
		...
		long gdipSelBackground = 0, gdipSelForeground = 0, gdipFont = 0,
				lastHFont = 0;
		long selBackground = 0;
		int selForeground = 0;
		if (hasSelection || ((flags & SWT.LAST_LINE_SELECTION) != 0 && (flags
				& (SWT.FULL_SELECTION | SWT.DELIMITER_SELECTION)) != 0)) {
			int fgSel = selectionForeground != null
					? selectionForeground.handle
					: OS.GetSysColor(OS.COLOR_HIGHLIGHTTEXT);
			int bgSel = selectionBackground != null
			...
			if (hasSelection) {
				selectionStart = translateOffset(
						Math.min(Math.max(0, selectionStart), length - 1));
				selectionEnd = translateOffset(
						Math.min(Math.max(0, selectionEnd), length - 1));
			}
		}
		RECT rect = new RECT();
		...
		for (int line = 0; line < runs.length; line++) {
			...
			int drawY = y
					+ DPIUtil.scaleUp(getDevice(), lineY[line], getZoom(gc));
			StyleItem[] lineRuns = runs[line];
			int lineHeight = DPIUtil.scaleUp(getDevice(),
					lineY[line + 1] - lineY[line] - lineSpacingInPoints,
					getZoom(gc));
			int lineHeightWithSpacing = DPIUtil.scaleUp(getDevice(),
					lineY[line + 1] - lineY[line], getZoom(gc));
			...
			if ((flags & (SWT.FULL_SELECTION | SWT.DELIMITER_SELECTION)) != 0
				...
				if (extents) {
					...
					if (gdip) {
						Gdip.Graphics_FillRectangle(gdipGraphics,
								gdipSelBackground,
								drawX + lineWidthInPixels[line], drawY, width,
								lineHeightWithSpacing);
					} else {
						OS.SelectObject(hdc, selBackground);
						OS.PatBlt(hdc, drawX + lineWidthInPixels[line], drawY,
								width, lineHeightWithSpacing, OS.PATCOPY);
					}
				}
			}
			...
			for (StyleItem run : lineRuns) {
				...
				if (drawX + run.width >= clip.x) {
					if (!run.lineBreak || run.softBreak) {
						if (extents) {
							OS.SetRect(rect, drawX, drawY, drawX + run.width,
									drawY + lineHeightWithSpacing);
						} else {
							OS.SetRect(rect, drawX, drawY, drawX + run.width,
									drawY + lineHeight);
						}
						if (gdip) {
							...
							drawRunBackground(run, hdc, rect, selectionStart,
									selectionEnd, selBackground, hasSelection);
						}
					}
				}
				drawX += run.width;
			...
			// Draw the text, underline, strikeout, and border of the runs in
			// the line
			int baselineInPixels = Math.max(0,
					DPIUtil.scaleUp(getDevice(), ascent, getZoom(gc)));
			int lineUnderlinePos = 0;
			for (StyleItem run : lineRuns) {
				baselineInPixels = Math.max(baselineInPixels, DPIUtil
						.scaleUp(getDevice(), run.ascentInPoints, getZoom(gc)));
				lineUnderlinePos = Math.min(lineUnderlinePos, run.underlinePos);
			}
			RECT borderClip = null, underlineClip = null, strikeoutClip = null,
					pRect = null;
			drawX = alignmentX;
			for (int i = 0; i < lineRuns.length; i++) {
				StyleItem run = lineRuns[i];
				TextStyle style = run.style;
				boolean hasAdorners = style != null && (style.underline
						|| style.strikeout || style.borderStyle != SWT.NONE);
				if (run.length == 0)
					continue;
				if (drawX > clip.x + clip.width)
					break;
					...
					if (!skipTab && (!run.lineBreak || run.softBreak)
						...
						if (gdip) {
							...
							if (gdipFont != 0 && !run.analysis.fNoGlyphIndex) {
								pRect = drawRunTextGDIP(gc, gdipGraphics, run,
										rect, gdipFont, baselineInPixels,
										gdipFg, gdipSelForeground,
										selectionStart, selectionEnd, alpha);
							} else {
								int fg = style != null && style.underline
										&& style.underlineStyle == SWT.UNDERLINE_LINK
												? linkColor
												: foreground;
								pRect = drawRunTextGDIPRaster(gc, gdipGraphics,
										run, rect, baselineInPixels, fg,
										selForeground, selectionStart,
										selectionEnd);
							}
							underlineClip = drawUnderlineGDIP(gc, gdipGraphics,
									x, drawY + baselineInPixels,
									lineUnderlinePos, drawY + lineHeight,
									lineRuns, i, gdipFg, gdipSelForeground,
									underlineClip, pRect, selectionStart,
									selectionEnd, alpha, clip);
							strikeoutClip = drawStrikeoutGDIP(gc, gdipGraphics,
									x, drawY + baselineInPixels, lineRuns, i,
									gdipFg, gdipSelForeground, strikeoutClip,
									pRect, selectionStart, selectionEnd, alpha,
									clip);
							...
							int fg = style != null && style.underline
									&& style.underlineStyle == SWT.UNDERLINE_LINK
											? linkColor
											: foreground;
							pRect = drawRunText(gc, hdc, run, rect,
									baselineInPixels, fg, selForeground,
									selectionStart, selectionEnd);
							underlineClip = drawUnderline(gc, hdc, x,
									drawY + baselineInPixels, lineUnderlinePos,
									drawY + lineHeight, lineRuns, i, fg,
									selForeground, underlineClip, pRect,
									selectionStart, selectionEnd, clip);
							strikeoutClip = drawStrikeout(gc, hdc, x,
									drawY + baselineInPixels, lineRuns, i, fg,
									selForeground, strikeoutClip, pRect,
									selectionStart, selectionEnd, clip);
	...
	RECT drawBorder(long hdc, int x, int y, int lineHeight, StyleItem[] line,
			int index, int color, int selectionColor, RECT clipRect, RECT pRect,
		...
		if (style.borderStyle == SWT.NONE)
			return null;
		clipRect = addClipRect(run, clipRect, pRect, selectionStart,
				selectionEnd);
		boolean lastRunVisible = drawClip != null
				&& (x + run.x + run.width) > (drawClip.x + drawClip.width);
		if (index + 1 >= line.length || lastRunVisible
				|| line[index + 1].lineBreak
				|| !style.isAdherentBorder(line[index + 1].style)) {
			int left = run.x;
			int start = run.start;
			int end = run.start + run.length - 1;
			for (int i = index; i > 0
					&& style.isAdherentBorder(line[i - 1].style); i--) {
				left = line[i - 1].x;
			...
			long oldBrush = OS.SelectObject(hdc,
					OS.GetStockObject(OS.NULL_BRUSH));
			LOGBRUSH logBrush = new LOGBRUSH();
			logBrush.lbStyle = OS.BS_SOLID;
			logBrush.lbColor = color;
			long newPen = OS.ExtCreatePen(lineStyle | OS.PS_GEOMETRIC,
					lineWidth, logBrush, 0, null);
			long oldPen = OS.SelectObject(hdc, newPen);
			RECT drawRect = new RECT();
			OS.SetRect(drawRect, x + left, y, x + run.x + run.width,
					y + lineHeight);
			if (drawClip != null) {
			...
			OS.Rectangle(hdc, drawRect.left, drawRect.top, drawRect.right,
					drawRect.bottom);
			...
			if (clipRect != null) {
				...
				long selPen = OS.ExtCreatePen(lineStyle | OS.PS_GEOMETRIC,
						lineWidth, logBrush, 0, null);
	...
	RECT drawBorderGDIP(long graphics, int x, int y, int lineHeight,
			StyleItem[] line, int index, long color, long selectionColor,
			RECT clipRect, RECT pRect, int selectionStart, int selectionEnd,
			int alpha, Rectangle drawClip) {
		StyleItem run = line[index];
		TextStyle style = run.style;
		if (style == null)
			return null;
		...
		if (index + 1 >= line.length || lastRunVisible
			...
			Gdip.Graphics_SetPixelOffsetMode(graphics,
					Gdip.PixelOffsetModeNone);
			...
			if (clipRect != null) {
				...
				Gdip.Graphics_SetClip(graphics, gdipRect,
						Gdip.CombineModeExclude);
				Gdip.Graphics_DrawRectangle(graphics, pen, x + left, y,
						run.x + run.width - left - 1, lineHeight - 1);
				Gdip.Graphics_Restore(graphics, gstate);
				gstate = Gdip.Graphics_Save(graphics);
				Gdip.Graphics_SetClip(graphics, gdipRect,
						Gdip.CombineModeIntersect);
				long selPen = Gdip.Pen_new(selectionColor, lineWidth);
				Gdip.Pen_SetDashStyle(selPen, lineStyle);
				Gdip.Graphics_DrawRectangle(graphics, selPen, x + left, y,
						run.x + run.width - left - 1, lineHeight - 1);
	...
	void drawRunBackground(StyleItem run, long hdc, RECT rect,
			int selectionStart, int selectionEnd, long selBrush,
		...
		if (fullSelection) {
			...
			OS.PatBlt(hdc, rect.left, rect.top, rect.right - rect.left,
					rect.bottom - rect.top, OS.PATCOPY);
		} else {
	...
	void drawRunBackgroundGDIP(StyleItem run, long graphics, RECT rect,
			int selectionStart, int selectionEnd, int alpha, long selBrush,
			boolean hasSelection) {
		int end = run.start + run.length - 1;
		boolean fullSelection = hasSelection && selectionStart <= run.start
				&& selectionEnd >= end;
		if (fullSelection) {
			Gdip.Graphics_FillRectangle(graphics, selBrush, rect.left, rect.top,
					rect.right - rect.left, rect.bottom - rect.top);
		} else {
			if (run.style != null && run.style.background != null) {
				long brush = createGdipBrush(run.style.background, alpha);
				Gdip.Graphics_FillRectangle(graphics, brush, rect.left,
			...
			if (partialSelection) {
				...
				Gdip.Graphics_FillRectangle(graphics, selBrush, rect.left,
						rect.top, rect.right - rect.left,
						rect.bottom - rect.top);
			}
		}
	...
	RECT drawRunText(GC gc, long hdc, StyleItem run, RECT rect,
			int baselineInPixels, int color, int selectionColor,
			int selectionStart, int selectionEnd) {
		int end = run.start + run.length - 1;
		boolean hasSelection = selectionStart <= selectionEnd
				&& selectionStart != -1 && selectionEnd != -1;
		...
		int y = rect.top + (baselineInPixels - DPIUtil.scaleUp(getDevice(),
				run.ascentInPoints, getZoom(gc)));
		long hFont = getItemFont(run, gc);
		...
		metricsAdapter.ScriptTextOut(hdc, run.psc, x, y, 0, null, run.analysis,
				0, 0, run.glyphs, run.glyphCount, run.advances, run.justify,
				run.goffsets);
		if (partialSelection) {
			getPartialSelection(run, selectionStart, selectionEnd, rect);
			OS.SetTextColor(hdc, selectionColor);
			metricsAdapter.ScriptTextOut(hdc, run.psc, x, y, OS.ETO_CLIPPED,
					rect, run.analysis, 0, 0, run.glyphs, run.glyphCount,
					run.advances, run.justify, run.goffsets);
		}
		return fullSelection || partialSelection ? rect : null;
	}

	RECT drawRunTextGDIP(GC gc, long graphics, StyleItem run, RECT rect,
			long gdipFont, int baselineInPixels, long color,
			long selectionColor, int selectionStart, int selectionEnd,
			int alpha) {
		int end = run.start + run.length - 1;
		boolean hasSelection = selectionStart <= selectionEnd
				&& selectionStart != -1 && selectionEnd != -1;
		...
		if (isMirrored) {
			switch (Gdip.Brush_GetType(brush)) {
				case Gdip.BrushTypeLinearGradient :
					Gdip.LinearGradientBrush_ScaleTransform(brush, -1, 1,
							Gdip.MatrixOrderPrepend);
					Gdip.LinearGradientBrush_TranslateTransform(brush,
							-2 * drawX - run.width, 0, Gdip.MatrixOrderPrepend);
					break;
				case Gdip.BrushTypeTextureFill :
					Gdip.TextureBrush_ScaleTransform(brush, -1, 1,
							Gdip.MatrixOrderPrepend);
					Gdip.TextureBrush_TranslateTransform(brush,
							-2 * drawX - run.width, 0, Gdip.MatrixOrderPrepend);
					break;
			}
			gstateMirrored = Gdip.Graphics_Save(graphics);
			Gdip.Graphics_ScaleTransform(graphics, -1, 1,
					Gdip.MatrixOrderPrepend);
			Gdip.Graphics_TranslateTransform(graphics, -2 * drawX - run.width,
					0, Gdip.MatrixOrderPrepend);
		}
		int[] advances = new int[run.glyphCount];
		float[] points = new float[run.glyphCount * 2];
		C.memmove(advances, run.justify != 0 ? run.justify : run.advances,
				run.glyphCount * 4);
		int glyphX = drawX;
		...
		Gdip.Graphics_DrawDriverString(graphics, run.glyphs, run.glyphCount,
				gdipFont, brush, points, 0, 0);
		if (partialSelection) {
			if (isMirrored) {
			...
			if (isMirrored) {
				...
				Gdip.Graphics_ScaleTransform(graphics, -1, 1,
						Gdip.MatrixOrderPrepend);
				Gdip.Graphics_TranslateTransform(graphics,
						-2 * drawX - run.width, 0, Gdip.MatrixOrderPrepend);
			}
			Gdip.Graphics_DrawDriverString(graphics, run.glyphs, run.glyphCount,
					gdipFont, selectionColor, points, 0, 0);
			Gdip.Graphics_Restore(graphics, gstate);
		}
		if (isMirrored) {
			switch (Gdip.Brush_GetType(brush)) {
	...
	RECT drawRunTextGDIPRaster(GC gc, long graphics, StyleItem run, RECT rect,
			int baselineInPixels, int color, int selectionColor,
			int selectionStart, int selectionEnd) {
		long clipRgn = 0;
		...
		OS.SetBkMode(hdc, OS.TRANSPARENT);
		RECT pRect = drawRunText(gc, hdc, run, rect, baselineInPixels, color,
	...
	RECT drawStrikeout(GC gc, long hdc, int x, int baselineInPixels,
			StyleItem[] line, int index, int color, int selectionColor,
			RECT clipRect, RECT pRect, int selectionStart, int selectionEnd,
			Rectangle drawClip) {
		StyleItem run = line[index];
		TextStyle style = run.style;
		...
		if (index + 1 >= line.length || lastRunVisible
				|| line[index + 1].lineBreak
				|| !style.isAdherentStrikeout(line[index + 1].style)) {
			int left = run.x;
			int start = run.start;
			int end = run.start + run.length - 1;
			for (int i = index; i > 0
					&& style.isAdherentStrikeout(line[i - 1].style); i--) {
				left = line[i - 1].x;
				start = Math.min(start, line[i - 1].start);
				end = Math.max(end, line[i - 1].start + line[i - 1].length - 1);
			}
			...
			RECT rect = new RECT();
			int riseInPixels = DPIUtil.scaleUp(getDevice(), style.rise,
					getZoom(gc));
			OS.SetRect(rect, x + left,
					baselineInPixels - run.strikeoutPos - riseInPixels,
					x + run.x + run.width, baselineInPixels - run.strikeoutPos
							+ run.strikeoutThickness - riseInPixels);
			...
			if (clipRect != null) {
				...
				if (clipRect.right == -1)
					clipRect.right = 0x7ffff;
				OS.SetRect(clipRect, Math.max(rect.left, clipRect.left),
						rect.top, Math.min(rect.right, clipRect.right),
						rect.bottom);
	...
	RECT drawStrikeoutGDIP(GC gc, long graphics, int x, int baselineInPixels,
			StyleItem[] line, int index, long color, long selectionColor,
			RECT clipRect, RECT pRect, int selectionStart, int selectionEnd,
			int alpha, Rectangle drawClip) {
		StyleItem run = line[index];
		...
		if (index + 1 >= line.length || lastRunVisible
			...
			if (clipRect != null) {
				...
				Gdip.Graphics_FillRectangle(graphics, brush, x + left,
						baselineInPixels - run.strikeoutPos - riseInPixels,
						run.x + run.width - left, run.strikeoutThickness);
				Gdip.Graphics_Restore(graphics, gstate);
				gstate = Gdip.Graphics_Save(graphics);
				Gdip.Graphics_SetClip(graphics, gdipRect,
						Gdip.CombineModeIntersect);
	...
	RECT drawUnderline(GC gc, long hdc, int x, int baselineInPixels,
			int lineUnderlinePos, int lineBottom, StyleItem[] line, int index,
			int color, int selectionColor, RECT clipRect, RECT pRect,
			int selectionStart, int selectionEnd, Rectangle drawClip) {
		StyleItem run = line[index];
		TextStyle style = run.style;
		if (style == null)
		...
		if (index + 1 >= line.length || lastRunVisible
				|| line[index + 1].lineBreak
				|| !style.isAdherentUnderline(line[index + 1].style)) {
			int left = run.x;
			int start = run.start;
			int end = run.start + run.length - 1;
			for (int i = index; i > 0
					&& style.isAdherentUnderline(line[i - 1].style); i--) {
				left = line[i - 1].x;
				start = Math.min(start, line[i - 1].start);
				end = Math.max(end, line[i - 1].start + line[i - 1].length - 1);
			}
			...
			OS.SetRect(rect, x + left,
					...
					x + run.x + run.width, baselineInPixels - lineUnderlinePos
							+ run.underlineThickness - riseInPixels);
			if (clipRect != null) {
				if (clipRect.left == -1)
					clipRect.left = 0;
				if (clipRect.right == -1)
			...
			switch (style.underlineStyle) {
				...
				case SWT.UNDERLINE_ERROR : {
					...
					int squigglyY = Math.min(rect.top - squigglyHeight / 2,
							lineBottom - squigglyHeight - 1);
					int[] points = computePolyline(rect.left, squigglyY,
							rect.right, squigglyY + squigglyHeight);
					long pen = OS.CreatePen(OS.PS_SOLID, squigglyThickness,
							color);
					long oldPen = OS.SelectObject(hdc, pen);
					int state = OS.SaveDC(hdc);
					OS.IntersectClipRect(hdc, rect.left, squigglyY,
							rect.right + 1, squigglyY + squigglyHeight + 1);
					OS.Polyline(hdc, points, points.length / 2);
					int length = points.length;
					if (length >= 2 && squigglyThickness <= 1) {
						OS.SetPixel(hdc, points[length - 2], points[length - 1],
								color);
					}
					OS.SelectObject(hdc, oldPen);
					OS.DeleteObject(pen);
					OS.RestoreDC(hdc, state);
					if (clipRect != null) {
						pen = OS.CreatePen(OS.PS_SOLID, squigglyThickness,
								selectionColor);
						oldPen = OS.SelectObject(hdc, pen);
						state = OS.SaveDC(hdc);
						OS.IntersectClipRect(hdc, clipRect.left, squigglyY,
								clipRect.right + 1,
								squigglyY + squigglyHeight + 1);
						OS.Polyline(hdc, points, points.length / 2);
						if (length >= 2 && squigglyThickness <= 1) {
							OS.SetPixel(hdc, points[length - 2],
									points[length - 1], selectionColor);
						}
						OS.SelectObject(hdc, oldPen);
						OS.DeleteObject(pen);
						OS.RestoreDC(hdc, state);
				...
				case UNDERLINE_IME_THICK :
					...
					int bottom = style.underlineStyle == SWT.UNDERLINE_DOUBLE
					...
					if (style.underlineStyle == SWT.UNDERLINE_DOUBLE) {
						OS.SetRect(rect, rect.left,
								rect.top + run.underlineThickness * 2,
								rect.right,
								rect.bottom + run.underlineThickness * 2);
						OS.FillRect(hdc, rect, brush);
					}
					...
					if (clipRect != null) {
						...
						OS.FillRect(hdc, clipRect, selBrush);
						if (style.underlineStyle == SWT.UNDERLINE_DOUBLE) {
							OS.SetRect(clipRect, clipRect.left, rect.top,
									clipRect.right, rect.bottom);
							OS.FillRect(hdc, clipRect, selBrush);
						}
						OS.DeleteObject(selBrush);
					}
				...
				case UNDERLINE_IME_DOT : {
					int penStyle = style.underlineStyle == UNDERLINE_IME_DASH
							? OS.PS_DASH
							: OS.PS_DOT;
					long pen = OS.CreatePen(penStyle, 1, color);
					long oldPen = OS.SelectObject(hdc, pen);
					int descentInPixels = DPIUtil.scaleUp(getDevice(),
							run.descentInPoints, getZoom(gc));
					OS.SetRect(rect, rect.left,
							baselineInPixels + descentInPixels, rect.right,
							baselineInPixels + descentInPixels
									+ run.underlineThickness);
	...
	RECT drawUnderlineGDIP(GC gc, long graphics, int x, int baselineInPixels,
			int lineUnderlinePos, int lineBottom, StyleItem[] line, int index,
			long color, long selectionColor, RECT clipRect, RECT pRect,
			int selectionStart, int selectionEnd, int alpha,
			Rectangle drawClip) {
		StyleItem run = line[index];
		TextStyle style = run.style;
		if (style == null)
			...
			switch (style.underlineStyle) {
				...
				case SWT.UNDERLINE_ERROR : {
					...
					if (gdipRect != null) {
						...
						Gdip.Graphics_SetClip(graphics, r,
								Gdip.CombineModeIntersect);
					}
					Gdip.Graphics_DrawLines(graphics, pen, points,
							points.length / 2);
					if (gdipRect != null) {
						long selPen = Gdip.Pen_new(selectionColor,
								squigglyThickness);
						Gdip.Graphics_Restore(graphics, gstate);
						gstate = Gdip.Graphics_Save(graphics);
						Gdip.Graphics_SetClip(graphics, gdipRect,
								Gdip.CombineModeIntersect);
						Gdip.Graphics_DrawLines(graphics, selPen, points,
								points.length / 2);
						Gdip.Pen_delete(selPen);
					}
					Gdip.Graphics_Restore(graphics, gstate);
				...
				case UNDERLINE_IME_THICK :
					...
					if (gdipRect != null) {
						...
						Gdip.Graphics_SetClip(graphics, gdipRect,
								Gdip.CombineModeIntersect);
						Gdip.Graphics_FillRectangle(graphics, selectionColor,
								rect.left, rect.top, rect.right - rect.left,
								rect.bottom - rect.top);
						if (style.underlineStyle == SWT.UNDERLINE_DOUBLE) {
							Gdip.Graphics_FillRectangle(graphics,
									selectionColor, rect.left,
									rect.top + run.underlineThickness * 2,
				...
				case UNDERLINE_IME_DASH : {
					...
					int dashStyle = style.underlineStyle == UNDERLINE_IME_DOT
							? Gdip.DashStyleDot
							: Gdip.DashStyleDash;
					...
					int descentInPixels = DPIUtil.scaleUp(getDevice(),
							run.descentInPoints, getZoom(gc));
					Gdip.Graphics_DrawLine(graphics, pen, rect.left,
							baselineInPixels + descentInPixels,
							run.width - run.length,
							baselineInPixels + descentInPixels);
					if (gdipRect != null) {
						Gdip.Graphics_Restore(graphics, gstate);
						gstate = Gdip.Graphics_Save(graphics);
						...
						Gdip.Graphics_DrawLine(graphics, selPen, rect.left,
								baselineInPixels + descentInPixels,
								run.width - run.length,
								baselineInPixels + descentInPixels);
						Gdip.Graphics_Restore(graphics, gstate);
	...
	public int getAscent() {
		checkLayout();
		System.out.println("getAscent: " + ascent);
		...
		if (wrapWidth != -1) {
			...
			for (int line = 0; line < runs.length; line++) {
				width = Math.max(width,
						DPIUtil.scaleDown(lineWidthInPixels[line], getZoom())
								+ getLineIndent(line));
			}
		}
		return new Rectangle(0, 0, width,
				lineY[lineY.length - 1] + getVerticalIndent());
	}
	...
	public Rectangle getBounds(int start, int end) {
		checkLayout();
		Rectangle r = DPIUtil.scaleDown(getDevice(),
				getBoundsInPixels(start, end), getZoom(null));

		System.out.println("getBounds(" + start + "," + end + "): " + r);
	...
	Rectangle getBoundsInPixels(int start, int end) {
		...
		/*
		 * use the high surrogate for the start offset and the low surrogate for
		 * the end offset
		 */
		...
		for (int i = 0; i < allRuns.length - 1; i++) {
			...
			if (run.start <= start && start < runEnd) {
				...
				if (run.style != null && run.style.metrics != null) {
					...
					cx = DPIUtil.scaleUp(getDevice(), metrics.width, getZoom())
							* (start - run.start);
				} else if (!run.tab) {
			...
			int lineIndex = 0;
			while (lineIndex < runs.length
					&& lineOffset[lineIndex + 1] <= run.start) {
				lineIndex++;
			}
			left = Math.min(left, runLead);
			right = Math.max(right, runTrail);
			top = Math.min(top,
					DPIUtil.scaleUp(lineY[lineIndex], getZoom(null)));
			bottom = Math.max(bottom, DPIUtil.scaleUp(
					lineY[lineIndex + 1] - lineSpacingInPoints, getZoom(null)));
		}
		return new Rectangle(left, top, right - left,
				bottom - top + getScaledVerticalIndent());
	}
	...
	public Rectangle getLineBounds(int lineIndex) {
		checkLayout();
		Rectangle r = DPIUtil.scaleDown(getDevice(),
				getLineBoundsInPixels(lineIndex), getZoom());

		System.out.println("getLineBounds(" + lineIndex + "): " + r);
	...
	Rectangle getLineBoundsInPixels(int lineIndex) {
		...
		int height = DPIUtil.scaleUp(getDevice(),
	...
	int getLineIndentInPixel(int lineIndex) {
		...
		if (wrapWidth != -1) {
			...
			if (partialLine) {
				...
				switch (alignment) {
					case SWT.CENTER :
						lineIndent += (DPIUtil.scaleUp(wrapWidth, getZoom())
								- lineWidth) / 2;
						break;
					case SWT.RIGHT :
						lineIndent += DPIUtil.scaleUp(wrapWidth, getZoom())
	...
	public int getLineIndex(int offset) {
		...
		for (int line = 0; line < runs.length; line++) {
			if (lineOffset[line + 1] > offset) {
				System.out.println("getLineIndex(" + offset + "):" + line);
				return line;
			}
		}

		System.out.println("getLineIndex(" + offset + ")runs.length-1:"
				+ (runs.length - 1));
	...
	public FontMetrics getLineMetrics(int lineIndex) {
		...
		int leadingInPoints = DPIUtil.scaleDown(getDevice(),
				lptm.tmInternalLeading, availableFont.zoom);
		if (text.length() != 0) {
		...
		lptm.tmDescent = DPIUtil.scaleUp(getDevice(), descentInPoints,
				getZoom());
		lptm.tmHeight = DPIUtil.scaleUp(getDevice(),
				ascentInPoints + descentInPoints, getZoom());
		lptm.tmInternalLeading = DPIUtil.scaleUp(getDevice(), leadingInPoints,
				getZoom());
		lptm.tmAveCharWidth = 0;

		FontMetrics fm = FontMetrics.win32_new(lptm, nativeZoom);

		System.out.println("getLineMetrics: " + lineIndex + "  " + fm);
	...
	public int[] getLineOffsets() {
		...
		System.out.println("getLineOffsets: " + Arrays.toString(offsets));
		return offsets;
	}
	...
	public Point getLocation(int offset, boolean trailing) {
		checkLayout();
		Point p = DPIUtil.scaleDown(getDevice(),
				getLocationInPixels(offset, trailing), getZoom());

		System.out.println("getLocation: " + offset + "  point:" + p);
	...
	Point getLocationInPixels(int offset, boolean trailing) {
		...
		if (offset == length) {
			return new Point(
					getLineIndentInPixel(line) + lineWidthInPixels[line],
					DPIUtil.scaleUp(getDevice(), lineY[line], getZoom()));
		}
		/*
		 * For trailing use the low surrogate and for lead use the high
		 * surrogate
		 */
	 ...
	 *            the first argument of OS.ScriptCPtoX
	 * @param trailing
	 *            the first argument of OS.ScriptCPtoX
	 * @param run
	 *            used to define remaining arguments of OS.ScriptCPtoX
	 * @return x position of the caret.
	 */
	private int ScriptCPtoX(int characterPosition, boolean trailing,
			StyleItem run) {
		int[] piX = new int[1];
		long advances = run.justify != 0 ? run.justify : run.advances;
		OS.ScriptCPtoX(characterPosition, trailing, run.length, run.glyphCount,
				run.clusters, run.visAttrs, advances, run.analysis, piX);
		return piX[0];
	}
	...
	public int getNextOffset(int offset, int movement) {
		checkLayout();
		int i = _getOffset(offset, movement, true);

		System.out.println("getNextOffset: " + offset + " movement " + movement
				+ "  out: " + i);
	...
	int _getOffset(int offset, int movement, boolean forward) {
		...
		int length = text.length();
		if (!(0 <= offset && offset <= length))
			SWT.error(SWT.ERROR_INVALID_RANGE, null,
					" [offset value: " + offset + "]");//$NON-NLS-1$ $NON-NLS-2$
		...
		do {
			...
			if (run.start <= offset && offset < run.start + run.length) {
				...
				OS.MoveMemory(properties, device.scripts[run.analysis.eScript],
						SCRIPT_PROPERTIES.sizeof);
				boolean isComplex = properties.fNeedsCaretInfo
						|| properties.fNeedsWordBreaking;
				if (isComplex)
					breakRun(run);
				while (run.start <= offset && offset < run.start + run.length) {
					if (isComplex) {
						OS.MoveMemory(logAttr,
								run.psla + ((offset - run.start)
										* SCRIPT_LOGATTR.sizeof),
								SCRIPT_LOGATTR.sizeof);
					}
					switch (movement) {
						...
						case SWT.MOVEMENT_WORD : {
							if (properties.fNeedsWordBreaking) {
								...
								if (offset > 0) {
									boolean letterOrDigit = Character
											.isLetterOrDigit(segmentsText
													.charAt(offset));
									boolean previousLetterOrDigit = Character
											.isLetterOrDigit(segmentsText
													.charAt(offset - 1));
									if (letterOrDigit != previousLetterOrDigit
											|| !letterOrDigit) {
										if (!Character.isWhitespace(
												segmentsText.charAt(offset))) {
											return untranslateOffset(offset);
										}
									}
								}
						...
						case SWT.MOVEMENT_WORD_END : {
							if (offset > 0) {
								boolean isLetterOrDigit = Character
										.isLetterOrDigit(
												segmentsText.charAt(offset));
								boolean previousLetterOrDigit = Character
										.isLetterOrDigit(segmentsText
			...
			i += step;
		} while (0 <= i && i < allRuns.length - 1 && 0 <= offset
				&& offset < length);
	...
	public int getOffset(Point point, int[] trailing) {
		...
		if (point == null)
			SWT.error(SWT.ERROR_NULL_ARGUMENT);
		return getOffsetInPixels(DPIUtil.scaleUp(getDevice(), point, getZoom()),
				trailing);
	}
	...
	public int getOffset(int x, int y, int[] trailing) {
		checkLayout();
		return getOffsetInPixels(DPIUtil.scaleUp(getDevice(), x, getZoom()),
				DPIUtil.scaleUp(getDevice(), y, getZoom()), trailing);
	}

	int getOffsetInPixels(int x, int y, int[] trailing) {
		computeRuns(null);

		System.out.println("getOffsetInPixels: " + x + "/" + y);
		...
		while (high - low > 1) {
			...
			if (run.x > x) {
				...
				if (run.style != null && run.style.metrics != null) {
					GlyphMetrics metrics = run.style.metrics;
					if (metrics.width > 0) {
						final int metricsWidthInPixels = DPIUtil
								.scaleUp(getDevice(), metrics.width, getZoom());
						if (trailing != null) {
							trailing[0] = (xRun
									% metricsWidthInPixels < metricsWidthInPixels
											/ 2) ? 0 : 1;
						}
						int res = untranslateOffset(
								run.start + xRun / metricsWidthInPixels);
				...
				if (run.tab) {
					...
					int res = untranslateOffset(run.start);
					System.out.println("Result: " + res);
					return res;
				}
				...
				long advances = run.justify != 0 ? run.justify : run.advances;
				OS.ScriptXtoCP(xRun, cChars, cGlyphs, run.clusters,
						run.visAttrs, advances, run.analysis, piCP, piTrailing);
		...
		int res =
				...
				untranslateOffset(lineOffset[line + 1]);
	...
	void getPartialSelection(StyleItem run, int selectionStart,
	...
	public int getPreviousOffset(int offset, int movement) {
		checkLayout();
		int r = _getOffset(offset, movement, false);
		System.out.println("getPreviousOffset" + offset + " mov: " + movement
				+ "  out: " + r);
	...
	public int[] getRanges() {
		...
		System.out.println("getRanges: " + Arrays.toString(result));
	...
	public int[] getSegments() {
		checkLayout();
		System.out.println("getSegments: " + Arrays.toString(segments));
	...
	public char[] getSegmentsChars() {
		checkLayout();
		System.out
				.println("getSegmentsChars: " + Arrays.toString(segmentsChars));
	...
	String getSegmentsText() {
		...
		char defaultSeparator = (resolveTextDirection()
				& SWT.RIGHT_TO_LEFT) != 0 ? RTL_MARK : LTR_MARK;
	...
	public int getSpacing() {
		checkLayout();
		System.out.println("getSpacing: " + lineSpacingInPoints);
	...
	public int getVerticalIndent() {
		checkLayout();
		System.out.println("getVerticalIndent: " + verticalIndentInPoints);
	...
	private int getScaledVerticalIndent() {
		...
		int r = DPIUtil.scaleUp(getDevice(), verticalIndentInPoints, getZoom());
		System.out.println("getScaledVerticalIndent: " + r);
		return r;
	}
	...
	public TextStyle getStyle(int offset) {
		...
		for (int i = 1; i < stylesCount; i++) {
			if (styles[i].start > offset) {
				...
				System.out.println("getStyle: " + offset + "  " + ts);
				return ts;
			}
		}
		System.out.println("getStyle: " + offset + "  null");
		return null;
	}
	...
	public TextStyle[] getStyles() {
		...
		System.out.println("getStyles: " + Arrays.toString(result));
	...
	public int[] getTabs() {
		...
		System.out.println("getTabs: " + Arrays.toString(tabs));
	...
	public int getWidth() {
		...
		System.out.println("getWidth: " + wrapWidth);
	...
	public int getWrapIndent() {
		checkLayout();
		System.out.println("getWrapIndent: " + wrapIndent);
	...
	StyleItem[] itemize() {
		...
		long hHeap = OS.GetProcessHeap();
		// This buffer needs to be one entry bigger than the cMaxItems param to
		// ScriptItemize
		// see
		// https://docs.microsoft.com/en-us/windows/win32/api/usp10/nf-usp10-scriptitemize
		// and https://bugzilla.mozilla.org/show_bug.cgi?id=366643 which was a
		// similar bug
		// in Mozilla. The MSDN docs have been updated since the Mozilla bug to
		// make this clear
		long pItems = OS.HeapAlloc(hHeap, OS.HEAP_ZERO_MEMORY,
				(1 + MAX_ITEM) * SCRIPT_ITEM.sizeof);
		if (pItems == 0)
			SWT.error(SWT.ERROR_NO_HANDLES);
		...
		if (BidiUtil.resolveTextDirection(text) != SWT.RIGHT_TO_LEFT) {
			/*
			 * With font ligatures enabled: CJK characters are not rendered
			 * properly when used in Java comments, workaround is to avoid
			 * ligatures between ascii and non-ascii chars. For more details
			 * refer bug 565526
			 */
			for (int i = 0, latestNeutralIndex = -2,
					latestUnicodeIndex = -2; i < length; i++) {
				char c = chars[i];
		...
		OS.ScriptItemize(chars, length, MAX_ITEM, scriptControl, scriptState,
				pItems, pcItems);
	...
	StyleItem[] merge(long items, int itemCount) {
		...
		/*
		 * Maximum size of runs is each itemized item + each style needing its
		 * own run + enough space for splitting runs that are too long.
		 */
		List<StyleItem> runs = new ArrayList<>(itemCount + stylesCount
				+ (end + MAX_RUN_LENGTH - 1) / MAX_RUN_LENGTH);
		...
		while (start < end) {
			...
			runs.add(item);
			OS.MoveMemory(scriptItem, items + itemIndex * SCRIPT_ITEM.sizeof,
					SCRIPT_ITEM.sizeof);
			...
			if (itemLimit == -1) {
				...
				OS.MoveMemory(scriptItem,
						items + nextItemIndex * SCRIPT_ITEM.sizeof,
						SCRIPT_ITEM.sizeof);
				itemLimit = scriptItem.iCharPos;
				if (nextItemIndex < itemCount && ch == '\r'
						&& segmentsText.charAt(itemLimit) == '\n') {
				...
				/*
				 * This block merges a bunch of tabs or non-complex scripts into
				 * a single item run. This is done so that less item runs are
				 * needed and is used when there could be a performance penalty
				 * because of too many runs. The tabs need to be "restored", see
				 * computeRuns
				 */
				if (nextItemIndex < itemCount && merge) {
					if (!item.lineBreak) {
						OS.MoveMemory(sp, device.scripts[item.analysis.eScript],
								SCRIPT_PROPERTIES.sizeof);
						if (!sp.fComplex || item.tab) {
							for (int i = 0; i < MERGE_MAX; i++) {
								...
								if (c == '\t' != item.tab)
									break;
								OS.MoveMemory(sp,
										device.scripts[scriptItem.a.eScript],
										SCRIPT_PROPERTIES.sizeof);
		...
		StyleItem item = new StyleItem();
		item.start = end;
		OS.MoveMemory(scriptItem, items + itemCount * SCRIPT_ITEM.sizeof,
				SCRIPT_ITEM.sizeof);
		item.analysis = scriptItem.a;
		runs.add(item);
	...
	/**
	 * Use OS.ScriptBreak to identify where in the run it is safe to split a
	 * character.
	 *
	 * @param run
	 *            the run to split
	...
	int splitLongRun(StyleItem run) {
		...
		for (int i = MAX_RUN_LENGTH - 1; i >= MAX_RUN_LENGTH
				- MAX_SEARCH_RUN_BREAK; i--) {
			int memoryIndex = i * SCRIPT_LOGATTR.sizeof;
			if (memoryIndex + SCRIPT_LOGATTR.sizeof > run.pslaAllocSize) {
				throw new IndexOutOfBoundsException();
			}
			OS.MoveMemory(logAttr, run.psla + memoryIndex,
					SCRIPT_LOGATTR.sizeof);
			if (logAttr.fSoftBreak || logAttr.fWhiteSpace
					|| logAttr.fWordStop) {
				best = i;
		...
		/*
		 * In the improbable case that the entire run has nowhere to split we
		 * need to make sure that at least we don't split a surrogate pair. This
		 * can happen if ScriptBreak above identifies nowhere that can be split,
		 * and the last character is the first part of a surrogate pair.
		 */
		if (Character
				.isHighSurrogate(segmentsText.charAt(run.start + best - 1))) {
			best--;
		}
	...
	int resolveTextDirection() {
		return textDirection == SWT.AUTO_TEXT_DIRECTION
	...
	StyleItem[] reorder(StyleItem[] runs, boolean terminate) {
		...
		/*
		 * Feature in Windows. If the orientation is RTL Uniscribe will resolve
		 * the level of line breaks to 1, this can cause the line break to be
		 * reorder to the middle of the line. The fix is to set the level to
		 * zero to prevent it to be reordered.
		 */
	...
	public void setAscent(int ascent) {
		System.out.println("setAscent: " + ascent);
		checkLayout();
		if (ascent < -1)
			SWT.error(SWT.ERROR_INVALID_ARGUMENT);
	...
	public void setSpacing(int spacing) {
		checkLayout();
		System.out.println("setSpacing: " + spacing);
		if (spacing < 0)
			SWT.error(SWT.ERROR_INVALID_ARGUMENT);
	...
	public void setStyle(TextStyle style, int start, int end) {
		...
		System.out.println("setStyle: " + style + " " + start + "  " + end);
		int length = text.length();
		if (length == 0)
			return;
		if (start > end)
		...
		if (modifyStart == modifyEnd) {
			...
			if (styleStart != start && styleEnd != end) {
				...
				System.out.println("setStyle: " + Arrays.toString(styles));
				return;
			}
		}
	...
	boolean shape(long hdc, StyleItem run, char[] chars, int[] glyphCount,
			int maxGlyphs, SCRIPT_PROPERTIES sp) {
		boolean useCMAPcheck = !sp.fComplex && !run.analysis.fNoGlyphIndex;
		if (useCMAPcheck) {
			short[] glyphs = new short[chars.length];
			if (OS.ScriptGetCMap(hdc, run.psc, chars, chars.length, 0,
					glyphs) != OS.S_OK) {
				if (run.psc != 0) {
		...
		int scriptShaprHr = OS.ScriptShape(hdc, run.psc, chars, chars.length,
				maxGlyphs, run.analysis, run.glyphs, run.clusters, run.visAttrs,
				glyphCount);
		if (scriptShaprHr == OS.S_OK) {
			...
			/*
			 * scriptShapeHr could have been OS.USP_E_SCRIPT_NOT_IN_FONT which
			 * indicates the whole run doesn't work with the font. The rest of
			 * this method verifies that none of the individual glyphs are
			 * missing an entry in the font. The fallback is to try other fonts
			 * (See caller)
			 */
	...
	long createMetafileWithChars(long hdc, long hFont, char[] chars,
			int charCount) {
		long hHeap = OS.GetProcessHeap();

		/*
		 * The native string must remain unchanged between ScriptStringAnalyse
		 * and ScriptStringOut. According to debugging, ScriptStringAnalyse
		 * implicitly saves string to SCRIPT_STRING_ANALYSIS. Then,
		 * ScriptStringOut uses the saved string in internal call to
		 * ExtTextOutW. I believe this is due to OS.SSA_METAFILE, which is
		 * documented as follows: Write items with ExtTextOutW calls, not with
		 * glyphs. Note: passing Java chars to native function is wrong, because
		 * JNI will allocate temporary native string which will be deallocated
		 * upon return from ScriptStringAnalyse.
		 */
		int nativeStringSize = charCount * Character.BYTES;
		long nativeString = OS.HeapAlloc(hHeap, OS.HEAP_ZERO_MEMORY,
				nativeStringSize);
		OS.MoveMemory(nativeString, chars, nativeStringSize);

		long ssa = OS.HeapAlloc(hHeap, OS.HEAP_ZERO_MEMORY,
				OS.SCRIPT_STRING_ANALYSIS_sizeof());
		long metaFileDc = OS.CreateEnhMetaFile(hdc, null, null, null);
		long oldMetaFont = OS.SelectObject(metaFileDc, hFont);
		int flags = OS.SSA_METAFILE | OS.SSA_FALLBACK | OS.SSA_GLYPHS
				| OS.SSA_LINK;
		if (OS.ScriptStringAnalyse(metaFileDc, nativeString, charCount, 0, -1,
				flags, 0, null, null, 0, 0, 0, ssa) == OS.S_OK) {
	...
	void shape(GC gc, final long hdc, final StyleItem run) {
		...
		if (run.clusters == 0)
			SWT.error(SWT.ERROR_NO_HANDLES);
		run.visAttrs = OS.HeapAlloc(hHeap, OS.HEAP_ZERO_MEMORY,
				maxGlyphs * SCRIPT_VISATTR_SIZEOF);
		if (run.visAttrs == 0)
			SWT.error(SWT.ERROR_NO_HANDLES);
		...
		if (!shapeSucceed) {
			...
			long newFont = 0;
			/*
			 * Bug in Uniscribe. In some version of Uniscribe,
			 * ScriptStringAnalyse crashes when the character array is too long.
			 * The fix is to limit the size of character array to two. Note,
			 * limiting the array to only one character would cause surrogate
			...
			for (int i = 0; i < chars.length; i++) {
				OS.MoveMemory(logAttr, run.psla + (i * SCRIPT_LOGATTR.sizeof),
						SCRIPT_LOGATTR.sizeof);
				if (!logAttr.fWhiteSpace) {
			...
			if (count > 0) {
				long metaFile = createMetafileWithChars(hdc, hFont, sampleChars,
						count);
				final EMREXTCREATEFONTINDIRECTW emr = new EMREXTCREATEFONTINDIRECTW();
				class MetaFileEnumProc {
					long metaFileEnumProc(long hDC, long table, long record,
							long nObj, long lpData) {
						OS.MoveMemory(emr.emr, record, EMR.sizeof);
						switch (emr.emr.iType) {
							case OS.EMR_EXTCREATEFONTINDIRECTW :
								OS.MoveMemory(emr, record,
										EMREXTCREATEFONTINDIRECTW.sizeof);
								break;
				...
				/*
				 * The run is composed only by white spaces, this happens when a
				 * run is split by a visual style. The font fallback for the
				 * script can not be determined using only white spaces. The
				 * solution is to use the font of the previous or next run of
				 * the same script.
				 */
				int index = 0;
						...
						if (newFont == 0) {
							if (index + 1 < allRuns.length - 1) {
								...
								if (nRun.analysis.eScript == run.analysis.eScript) {
									...
									LOGFONT logFont = new LOGFONT();
									OS.GetObject(nFont, LOGFONT.sizeof,
											logFont);
									newFont = OS.CreateFontIndirect(logFont);
								}
							}
			...
			if (newFont != 0) {
				...
				if (shapeSucceed = shape(hdc, run, chars, buffer, maxGlyphs,
						sp)) {
			...
			if (!shapeSucceed) {
				if (mLangFontLink2 != null) {
					...
					mLangFontLink2.GetStrCodePages(chars, chars.length, 0,
							dwCodePages, cchCodePages);
					if (mLangFontLink2.MapFont(hdc, dwCodePages[0], chars[0],
							hNewFont) == OS.S_OK) {
						LOGFONT logFont = new LOGFONT();
						...
						if (shapeSucceed = shape(hdc, run, chars, buffer,
								maxGlyphs, sp)) {
		...
		if (!shapeSucceed) {
			/*
			 * Shape Failed. Give up and shape the run with the default font.
			 * Missing glyphs typically will be represent as black boxes in the
			 * text.
			 */
			OS.ScriptShape(hdc, run.psc, chars, chars.length, maxGlyphs,
					run.analysis, run.glyphs, run.clusters, run.visAttrs,
					buffer);
			run.glyphCount = buffer[0];
		}
		int[] abc = new int[3];
		run.advances = OS.HeapAlloc(hHeap, OS.HEAP_ZERO_MEMORY,
				run.glyphCount * 4);
		if (run.advances == 0)
			SWT.error(SWT.ERROR_NO_HANDLES);
		run.goffsets = OS.HeapAlloc(hHeap, OS.HEAP_ZERO_MEMORY,
				run.glyphCount * GOFFSET_SIZEOF);
		if (run.goffsets == 0)
			SWT.error(SWT.ERROR_NO_HANDLES);
		OS.ScriptPlace(hdc, run.psc, run.glyphs, run.glyphCount, run.visAttrs,
		...
		if (style != null) {
			...
			if (style.underline || style.strikeout) {
				...
				if (metricsAdapter.GetOutlineTextMetrics(hdc,
						OUTLINETEXTMETRIC.sizeof, lotm) == 0) {
					lotm = null;
				}
			}
			if (style.metrics != null) {
				...
				/*
				 * Bug in Windows, on a Japanese machine, Uniscribe returns
				 * glyphcount equals zero for FFFC (possibly other unicode code
				 * points), the fix is to make sure the glyph is at least one
				 * pixel wide.
				 */
				run.width = DPIUtil.scaleUp(getDevice(), metrics.width,
						getZoom()) * Math.max(1, run.glyphCount);
				...
				run.descentInPoints = DPIUtil.scaleDown(getDevice(),
						lptm.tmDescent, getZoom(gc));
				run.leadingInPoints = DPIUtil.scaleDown(getDevice(),
						lptm.tmInternalLeading, getZoom(gc));
			}
			if (lotm != null) {
				...
				run.strikeoutPos = DPIUtil.scaleUp(getDevice(),
						run.ascentInPoints, getZoom(gc)) / 2;
			...
			TEXTMETRIC lptm = new TEXTMETRIC();
			metricsAdapter.GetTextMetrics(hdc, lptm);
			run.ascentInPoints = DPIUtil.scaleDown(getDevice(), lptm.tmAscent,
					getZoom(gc));
			run.descentInPoints = DPIUtil.scaleDown(getDevice(), lptm.tmDescent,
					getZoom(gc));
			run.leadingInPoints = DPIUtil.scaleDown(getDevice(),
					lptm.tmInternalLeading, getZoom(gc));
		}
