...
private class AddPathOperation implements Operation {
	...
	@Override
	public void apply(PathHandle pathHandle) {
		applyOnTemporaryHandle(getDevice(), pathHandle.zoom, temporaryHandle -> {
			this.operations.forEach(op -> op.apply(temporaryHandle));
...
private <T> T applyOnAnyHandle(Function<PathHandle, T> function) {
	if (zoomToHandle.isEmpty()) {
		return applyOnTemporaryHandle(getDevice(), DPIUtil.getDeviceZoom(), function);
	} else {
		return function.apply(zoomToHandle.values().iterator().next());
	}
}

private static <T> T applyOnTemporaryHandle(Device device, int zoom, Function<PathHandle, T> function) {
