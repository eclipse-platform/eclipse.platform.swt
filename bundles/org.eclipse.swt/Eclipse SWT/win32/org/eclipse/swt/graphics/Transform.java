...
public void multiply(Transform matrix) {
	...
	storeAndApplyOperationForAllHandles(new MultiplayOperation(matrix));
}
...
private class TransformHandle {
	...
	public TransformHandle(long handle, int zoom) {
		this.handle = handle;
		this.zoom = zoom;
	}
...
private class InvertOperation implements Operation {
...
private class RotateOperation implements Operation {
	private float angle;
...
private class ScaleOperation implements Operation {
	private float scaleX;
	private float scaleY;

	public ScaleOperation(float scaleX, float scaleY) {
		this.scaleX = scaleX;
		this.scaleY = scaleY;
	}
...
private class ShearOperation implements Operation {
	private float shearX;
	private float shearY;

	public ShearOperation(float shearX, float shearY) {
		this.shearX = shearX;
		this.shearY = shearY;
	}
...
private <T> T applyOnAnyHandle(Function<TransformHandle, T> function) {
	if (zoomToHandle.isEmpty()) {
		TransformHandle handle = newTransformHandle(DPIUtil.getDeviceZoom());
		try {
...
private TransformHandle getTransformHandle(int zoom) {
	if (!zoomToHandle.containsKey(zoom)) {
		TransformHandle newPathHandle = newTransformHandle(zoom);
