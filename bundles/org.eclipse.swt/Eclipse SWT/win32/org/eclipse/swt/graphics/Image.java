...
public final class Image extends Resource implements Drawable {
	...
	private class ImageHandleManager {
		...
		ImageHandle getImageHandle(int zoom) {
		...
		ImageHandle getHandleOrCreate(int zoom, Supplier<ImageHandle> creator) {
			ImageHandle imageHandle = zoomLevelToImageHandle.computeIfAbsent(zoom, __ -> {
				if (zoom == -1) {
					return null;
				}
				ImageHandle imageHandleToReturn = creator.get();
				return imageHandleToReturn;
			});
			return imageHandle;
		}

		boolean hasImageHandle(ImageHandle imageHandle) {
		    return zoomLevelToImageHandle.values().contains(imageHandle);
		}

		boolean hasZoom(int zoom) {
		...
		List<ImageHandle> getAllImageHandles() {
			return zoomLevelToImageHandle.values().stream().toList();
		}
		...
		void register(ImageHandle imageHandle) {
			imageHandles.add(imageHandle);
		}

		private void cleanupDanglingImageHandles() {
			Collection<ImageHandle> mappedImageHandles = zoomLevelToImageHandle.values();
			List<ImageHandle> danglingImageHandles = imageHandles.stream()
					.filter(imageHandle -> !mappedImageHandles.contains(imageHandle))
					.peek(imageHandle -> imageHandle.destroy())
					.toList();
			imageHandles.removeAll(danglingImageHandles);
		}
	...
	private class HandleAtSize {
		...
		public void destroy() {
			if (handleContainer != null && !imageHandleManager.hasImageHandle(handleContainer)) {
				imageHandleManager.destroy(handleContainer);
			}
		...
		private ImageHandle getOrCreateImageHandleAtClosestSize(int widthHint, int heightHint) {
			...
			ImageHandle bestFittingHandle = imageHandleManager.getImageHandle(imageZoom);
...
private ImageHandle getImageMetadata(ZoomContext zoomContext) {
	...
	return imageHandleManager.getHandleOrCreate(targetZoom, () -> imageProvider.newImageHandle(zoomContext));
}
...
Rectangle getBounds(int zoom) {
	if (isDisposed()) SWT.error(SWT.ERROR_GRAPHIC_DISPOSED);
	if (imageHandleManager.hasZoom(zoom)) {
		ImageHandle imageMetadata = imageHandleManager.getImageHandle(zoom);
...
private void checkImageTypeForValidCustomDrawing(int zoom) {
	...
	} else if (!imageHandleManager.hasNoBaseHandle()
			&& (imageHandleManager.getAllImageHandles().size() != 1 || !imageHandleManager.hasZoom(zoom))) {
...
<T> T applyUsingAnyHandle(Function<ImageHandle, T> function) {
	if (imageHandleManager.hasNoBaseHandle()) {
...
private class ImageFileNameProviderWrapper extends BaseImageProviderWrapper<ImageFileNameProvider> {
	...
	protected ElementAtZoom<ImageData> loadImageData(int zoom) {
		...
		if (imageHandleManager.hasZoom(fileForZoom.zoom())) {
			nativeInitializedImage = imageHandleManager.getImageHandle(fileForZoom.zoom());
		} else {
		...
		if (nativeInitializedImage == null) {
			...
			imageHandleManager.destroy(nativeInitializedImage);
		}
...
class ImageHandle {
	...
	ImageHandle(long handle, int zoom, int transparentPixel) {
		...
		imageHandleManager.register(this);
	}
