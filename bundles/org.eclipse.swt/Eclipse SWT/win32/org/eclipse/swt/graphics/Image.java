...
public Image (Device device, String filename) {
	...
	this.imageProvider = new ImageFileProviderWrapper(zoom -> {
		return zoom == 100 ? file : null;
	});
	if (imageProvider.getImageData(100) == null) {
		SWT.error(SWT.ERROR_INVALID_ARGUMENT, null, ": [" + file + "] returns null ImageData at 100% zoom.");
	}
	init();
	this.device.registerResourceWithZoomSupport(this);
}
...
public Image(Device device, ImagePathProvider imageFileProvider) {
	super(device);
	this.imageProvider = new ImageFileProviderWrapper(imageFileProvider);
	initialNativeZoom = DPIUtil.getNativeDeviceZoom();
	if (imageProvider.getImageData(100) == null) {
...
private class ImageFileProviderWrapper extends BaseImageProviderWrapper<ImagePathProvider> {
	ImageFileProviderWrapper(ImagePathProvider provider) {
	...
	@Override
	ImageFileProviderWrapper createCopy(Image image) {
		return image.new ImageFileProviderWrapper(provider);
	}
