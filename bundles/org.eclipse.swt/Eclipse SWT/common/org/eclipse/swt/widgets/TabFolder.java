...
public class TabFolder extends Composite {
	...
	TabFolder2Listener[] folderListeners = new TabFolder2Listener[0];
	// support for deprecated listener mechanism
	TabFolderListener[] tabListeners = new TabFolderListener[0];
...
void init(int style) {
	super.setLayout(new TabFolderLayout());
...
//	showMax = (style2 & SWT.MAX) != 0; - conflicts with SWT.BOTTOM
	...
	renderer = rendererFactory.createTabFolderRenderer(this);
 ...
 * @see #removeCTabFolder2Listener(TabFolder2Listener)
 *
 * @since 3.0
 */
 ...
 * </ul>
 *
 * @see TabFolderListener
 * @see #removeCTabFolderListener(TabFolderListener)
 *
...
Rectangle[] computeControlBounds(Point size, boolean[][] position) {
	...
	Rectangle bodyTrim = renderer.computeTrim(TabFolderRenderer.PART_BODY, SWT.NONE, 0, 0, 0, 0);
	...
	if (showChevron) {
		...
		if (lastIndex != -1) {
			TabItem lastItem = items[lastIndex];
...
private void notifyItemCountChange() {
	TabFolderEvent e = new TabFolderEvent(this);
	for (TabFolder2Listener listener : folderListeners) {
...
public Rectangle getClientArea() {
	...
	Rectangle trim = renderer.computeTrim(TabFolderRenderer.PART_BODY, SWT.FILL, 0, 0, 0, 0);
	Point size = getSize();
	int wrapHeight = getWrappedHeight(size);
	if (onBottom) {
		trim.height += wrapHeight;
 ...
 *	</ul>
 */
public TabItem getItem(Point pt) {
	/*
	 * This call is intentionally commented out, to allow this getter method to be
	 * called from a thread which is different from one that created the widget.
	 */
...
public TabItem[] getSelection() {
	...
	TabItem[] selected = new TabItem[1];
			...
			if (!mru) {
				...
				if (current + offset >= 0 && current + offset < idx){
					...
					if (showChevron) {
						...
						e.doit = true;
						for (TabFolder2Listener folderListener : folderListeners) {
	...
	if (event.detail == SWT.MENU_KEYBOARD) {
		if (selectedIndex != -1) {
			...
			if (!rect.contains(event.x, event.y)) {
				...
				Rectangle closeTrim = renderer.computeTrim(TabFolderRenderer.PART_CLOSE_BUTTON, SWT.NONE, 0, 0, 0, 0);
								...
								for (Control control : controls) {
									...
									do {
										if (temp.equals(control)) {
											...
											temp = temp.getParent();
											if (temp == null || temp.equals(TabFolder.this))
												break;
										}
									} while (!reschedule);
									if (reschedule) break;
		...
		case SWT.MouseUp: {
			...
			if (item != null) {
				if (item.closeRect.contains(x,y)) {
					...
					for (TabFolderListener listener : tabListeners) {
 ...
 * </ul>
 *
 * @see #addCTabFolder2Listener(TabFolder2Listener)
 *
 * @since 3.0
 */
public void removeCTabFolder2Listener(TabFolder2Listener listener) {
	...
	if (folderListeners.length == 1) {
		folderListeners = new TabFolder2Listener[0];
		return;
	}
	TabFolder2Listener[] newTabListeners = new TabFolder2Listener[folderListeners.length - 1];
...
public void removeCTabFolderListener(TabFolderListener listener) {
	...
	if (tabListeners.length == 1) {
		tabListeners = new TabFolderListener[0];
		return;
	}
	TabFolderListener[] newTabListeners = new TabFolderListener[tabListeners.length - 1];
...
void updateButtons() {
	...
	if (showMax) {
		...
		if (maxItem == null) {
			...
			if (maxImage == null) {
				maxImage = createButtonImage(display, TabFolderRenderer.PART_MAX_BUTTON);
			}
	...
	if (showMin) {
		...
		if (minItem == null) {
			...
			if (minImage == null) {
				minImage = createButtonImage(display, TabFolderRenderer.PART_MIN_BUTTON);
			}
...
private void updateChevronImage(boolean styleChange) {
	...
	if (chevronImage != null) chevronImage.dispose();
	chevronImage = createButtonImage(getDisplay(), TabFolderRenderer.PART_CHEVRON_BUTTON);
...
boolean setItemLocation(GC gc) {
	...
	Point closeButtonSize = renderer.computeSize(TabFolderRenderer.PART_CLOSE_BUTTON, 0, gc, SWT.DEFAULT, SWT.DEFAULT);
	int leftItemEdge = getLeftItemEdge(gc, TabFolderRenderer.PART_BORDER);
	if (single) {
		...
		for (int i = 0; i < priority.length; i++) {
			TabItem item = items[priority[i]];
			width += item.width;
			item.showing = i == 0 ? true : item.width > 0 && width <= maxWidth;
		}
		int x = getLeftItemEdge(gc, TabFolderRenderer.PART_HEADER);
		int defaultX = getDisplay().getBounds().width + 10; // off screen
...
boolean setItemSize(GC gc) {
	...
	if (single) {
		...
		if (selectedIndex != -1) {
			...
			width = Math.min(width, getRightItemEdge(gc) - getLeftItemEdge(gc, TabFolderRenderer.PART_BORDER));
			if (tab.height != tabHeight || tab.width != width) {
				...
				if (showClose || tab.showClose) {
					Point closeSize = renderer.computeSize(TabFolderRenderer.PART_CLOSE_BUTTON, SWT.SELECTED, gc, SWT.DEFAULT, SWT.DEFAULT);
	...
	int[] widths;
	int tabAreaWidth = Math.max(0, getRightItemEdge(gc) - getLeftItemEdge(gc, TabFolderRenderer.PART_BORDER));
	// First, try the minimum tab size at full compression.
	int minWidth = 0;
	int[] minWidths = new int[items.length];
	for (int element : priority) {
		int index = element;
		int state = TabFolderRenderer.MINIMUM_SIZE;
	...
	for (int i = 0; i < items.length; i++) {
		...
		if (showClose || tab.showClose) {
			if (i == selectedIndex || showUnselectedClose) {
				Point closeSize = renderer.computeSize(TabFolderRenderer.PART_CLOSE_BUTTON, SWT.NONE, gc, SWT.DEFAULT, SWT.DEFAULT);
				tab.closeRect.width = closeSize.x;
				tab.closeRect.height = closeSize.y;
			}
		}
...
public void setMaximized(boolean maximize) {
	...
	if (minMaxTb != null && maxItem != null) {
		if (maxImage != null) maxImage.dispose();
		maxImage = createButtonImage(getDisplay(), TabFolderRenderer.PART_MAX_BUTTON);
		maxItem.setImage(maxImage);
		maxItem.setToolTipText(maximized ? SWT.getMessage("SWT_Restore") : SWT.getMessage("SWT_Maximize")); //$NON-NLS-1$ //$NON-NLS-2$
	}
}
...
public void setMinimized(boolean minimize) {
	...
	if (minMaxTb != null && minItem != null) {
		if (minImage != null) minImage.dispose();
		minImage = createButtonImage(getDisplay(), TabFolderRenderer.PART_MIN_BUTTON);
		minItem.setImage(minImage);
		minItem.setToolTipText(minimized ? SWT.getMessage("SWT_Restore") : SWT.getMessage("SWT_Minimize")); //$NON-NLS-1$ //$NON-NLS-2$
	}
}
 ...
 * @since 3.6
 */
public void setRenderer(TabFolderRenderer renderer) {
	checkWidget();
 ...
 *
 * @param thickness the desired thickness. Must be positive and lower than {@link TabFolderRenderer#ITEM_TOP_MARGIN} (that is either {@code 0} {@code 1} or {@code 2} at the moment),for correct results.
 ...
 * </ul>
 * @implNote Default {@link TabFolderRenderer} currently ignores this setting if {@link #getSimple()} is {@code true}.
 * @since 3.121
 */
	...
	for (TabItem tab : items) {
		...
		item.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				int index = indexOf((TabItem) menuItem.getData(id));
				TabFolder.this.setSelection(index, true);
			}
		});
	}
	int x = rect.x;
 ...
 *                         </ul>
 *
 * @see TabFolder#showItem(TabItem)
 *
 * @since 2.0
 */
...
boolean updateItems(int showIndex) {
	...
	if (!single && !mru && showIndex != -1) {
		...
		if (priority[0] < showIndex) {
			int maxWidth = getRightItemEdge(gc) - getLeftItemEdge(gc, TabFolderRenderer.PART_BORDER);
			int width = 0;
...
boolean updateTabHeight(boolean force){
	...
	GC gc = new GC(this);
	tabHeight = renderer.computeSize(TabFolderRenderer.PART_HEADER, SWT.NONE, gc, SWT.DEFAULT, SWT.DEFAULT).y;
	gc.dispose();
				...
				if (hovering) {
					...
					if (wrapped || gradientColors == null) {
						...
						if (colorChanged || !bounds.equals(bkImageBounds[i])) {
							...
							controlBkImages[i] = new Image(control.getDisplay(), (gc, imageWidth, imageHeight) -> renderer.draw(TabFolderRenderer.PART_BACKGROUND, 0, bounds, gc), bounds.width, bounds.height);
...
public void setSelection(TabItem[] tabItems) {
	checkWidget();
	if (tabItems == null) {
		throw new IllegalArgumentException("Selection being set cannot be null");
	}
	if (items == null)
	...
	if (items.length == 0) {
		...
		for (int i = tabItems.length - 1; i >= 0; --i) {
			int index = indexOf(tabItems[i]);
			if (index != -1)
...
public void setHoverRect(Rectangle hoverRect) {
