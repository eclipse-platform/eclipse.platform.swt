...
import org.eclipse.swt.pty.Spawner.*;
...
//			outInit = new ConPTYOutputStream(conPTY);
			//FIXME separate fragemnt for conpyty
		} else {
	...
	/**
	 * Spawns a new process using this PTY
	 * @param cmdarray
	 * @param envp
	 * @param dir
	 * @return the Process
	 * @throws IOException
	 */
	public Spawner spawnProcess(String[] cmdarray, String[] envp, File dir) throws IOException {
		return (Spawner) ProcessFactory.getFactory().exec(cmdarray, envp, dir, this);
	}
	...
	public final void setTerminalSize(int width, int height) {
		try {
			if (ptyType == PTY_TYPE.PTY_CONPTY) {
//FIXME				conPTY.setTerminalSize(width, height);
			} else {
			...
			if (!setTerminalSizeErrorAlreadyLogged) {
				...
				System.err.println(Messages.Util_exception_cannotSetTerminalSize+":"+ e);
			}

		}
	}

	int exec_pty(Spawner spawner, String[] cmdarray, String[] envp, String dir, IChannel[] chan)
			throws IOException {
		if (ptyType == PTY_TYPE.PTY_CONPTY) {
//FIXME			return conPTY.exec(cmdarray, envp, dir);
			return -1;
	...
	int waitFor(Spawner spawner, int pid) {
		if (ptyType == PTY_TYPE.PTY_CONPTY) {
...
//				}
			...
			if (ptyType == PTY_TYPE.PTY_UNKNOWN) {
				try {
					...
					System.err.println(Messages.PTY_FailedToStartWinPTY+":"+ e);
				}
			}
		} else {
			try {
				...
				System.err.println(Messages.PTY_FailedToStartNativePTY+":"+ e);
			}
		}
		...
		if (ptyType == PTY_TYPE.PTY_UNKNOWN) {
			...
			isConsoleModeSupported = true;
			System.err.println(Messages.PTY_FailedToStartPTY);
		}
