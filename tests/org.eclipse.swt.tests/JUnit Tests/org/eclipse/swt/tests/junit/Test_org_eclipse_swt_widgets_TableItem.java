...
public class Test_org_eclipse_swt_widgets_TableItem extends Test_org_eclipse_swt_widgets_Item {
	...
	public void test_getBoundsIV() {
		...
		var bounds = table.getItem(9999).getBounds();

		assertTrue(" item 10000, should have position at least 1000 ",bounds.y >= 1000);


		table.setTopIndex(9999);

		bounds = table.getItem(0).getBounds();

		assertTrue(" item 0, with topindex 9999, should have position smaller than -1000 ",bounds.y <= -1000);
	...
	@Test
	public void test_setVerticalBarSelection() {
		...
		table.getVerticalBar().setSelection(5);
		table.getVerticalBar().notifyListeners(SWT.Selection, new Event());
		assertEquals(5, table.getTopIndex());

		table.setTopIndex(0);
		assertEquals(0, table.getVerticalBar().getSelection());

	}

	@Test
	public void test_getBoundsIII() {

		final int TOP_INDEX = 5;
		...
		for (int i = 1; i < table.getItemCount(); i++) {
			...
			var prev = table.getItem(i - 1).getBounds();

			var it = table.getItem(i).getBounds();

			assertEquals(prev.x, it.x);
			assertEquals(prev.width, it.width);

			assertTrue(prev.y + prev.height == it.y);
			...
			if (i == TOP_INDEX) {
				assertTrue(prev.y < 0);
				assertTrue(it.y == 0 || it.y == 1);
			}

			if (i == TOP_INDEX + 1) {
				assertTrue(it.y > 0);
				assertTrue(prev.y == 0 || prev.y == 1);
			} else if (i > TOP_INDEX) {
				assertTrue(it.y > 0);
				assertTrue(prev.y > 0);
			}
			...
			for (int c = 0; c < table.getColumnCount(); c++) {
				...
				var col = table.getColumn(c);

				var cellBound = table.getItem(i).getBounds(c);

				assert (it.x <= cellBound.x);
				assert (it.width >= cellBound.width);
				assert (it.y <= cellBound.y);
				assert (it.height >= cellBound.height);

				assertContains(it, cellBound);

				if (c == 0)
					assertTrue(cellBound.x > widthPosition);
				else {
					assertEquals(cellBound.x, widthPosition);
				}

				widthPosition += col.getWidth();
				assertEquals(cellBound.x + cellBound.width, widthPosition);
	...
	private void assertContains(Rectangle out, Rectangle in) {

		// diagonal points are sufficient. Both are rectangles
		assertRectangleContains(out, in.x, in.y);
		assertRectangleContains(out, in.x + in.width, in.y + in.height);

	}

	private void assertRectangleContains(Rectangle r, int x, int y) {

		assertTrue(x >= r.x);
		assertTrue(y >= r.y);
		assertTrue(x <= r.x + r.width);
		assertTrue(y <= r.y + r.height);
	...
	public void test_getBoundsI() {
		if (SwtTestUtil.isGTK || SwtTestUtil.isCocoa) {
			...
			if (SwtTestUtil.verbose) {
				System.out.println(
						"Excluded test_getBoundsI(org.eclipse.swt.tests.junit.Test_org_eclipse_swt_widgets_TableItem)");
			}
			return;
		}
...
//	assertTrue(":1i:", bounds2.width > bounds.width); // TODO once an image is added the space for it is always there
		...
		// assertTrue(":2i:", bounds2.width < bounds.width); // TODO once an image is
		// added the space for it is always there
		...
		bounds = tableItem.getBounds(1);
		assertTrue(":3b:", /* bounds.x > 0 && */ bounds.height > 0 && bounds.width == 0); // TODO bounds.x == 0 Is this
																							// right?
		bounds = tableItem.getBounds(-1);
		...
		bounds = tableItem2.getBounds(1);
		assertTrue(":4b:", /* bounds.x > 0 && */ bounds.height > 0 && bounds.width == 0); // TODO bounds.x == 0 Is this
																							// right?
		bounds = tableItem2.getBounds(-1);
...
//	imageHeight = table2.getItemHeight() - table2.getGridLineWidth();
		assertEquals(new Rectangle(0, 0, 0, 0), tableItem2.getImageBounds(-1));
		bounds = tableItem2.getImageBounds(0); // bounds.width should be check box width if check box is wider than
												// image
//	assertTrue(":b:", bounds.x > 0 && bounds.width > 0 && bounds.height == imageHeight);
		assertEquals(new Rectangle(0, 0, 0, 0), tableItem2.getImageBounds(1));
	}
	...
	public void test_setImage$Lorg_eclipse_swt_graphics_Image() {
		...
		// table.setText(texts); // create enough columns for
		// TableItem.setImage(Image[]) to work
	...
	public void test_setText$Ljava_lang_String() {
		...
		/*
		 * Test the getText/setText API with a Table that has only the default column.
		 */
		...
		/*
		 * Test the getText/setText API with a Table that enough columns to fit all test
		 * item texts.
		 */
	...
	public void test_setTextILjava_lang_String() {
		...
		// table.setText(TestStrings); // create anough columns for
		// TableItem.setText(String[]) to work
		int columnCount = table.getColumnCount();
		if (columnCount < images.length) {
			for (int i = columnCount; i < images.length; i++) {
				new TableColumn(table, SWT.NONE);
		...
		/*
		 * Test the getText/setText API with a small table that will be extended by a
		 * column: First the table has only one column and one row (TableItem) with
		 * text. Then it is extended by one more column. After this the existing row
		 * gets one more text for the new column.
		 */
