...
public class Snippet388 {
	...
	public static void main(String[] args) {
		...
		canvas.addListener(SWT.Paint, e -> {
			GC gc = e.gc;
			printContent(display, gc);
		});

		Button exportPrintButton = new Button(shell, SWT.PUSH);
		exportPrintButton.setText("Shell#print to PDF");
		exportPrintButton.setLayoutData(new BorderData(SWT.BOTTOM));
		exportPrintButton.addListener(SWT.Selection, e -> {
			generatePdf(display, shell, shell::print);
		});
		Button exportRenderButton = new Button(shell, SWT.PUSH);
		exportRenderButton.setText("Render with GC to PDF");
		exportRenderButton.setLayoutData(new BorderData(SWT.BOTTOM));
		exportRenderButton.addListener(SWT.Selection, e -> {
			generatePdf(display, shell, gc -> printContent(display, gc));
		});
	...
	private static void generatePdf(Display display, Shell shell, Consumer<GC> printFunction) {
		try {
	...
	private static void printContent(Display display, GC gc) {
